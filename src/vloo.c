...
{
    particle_t super;
    void (*set)(struct vloo_particle_t* self, 
		float u, float v, float w, float s, double birth_ms);
...
static
void set_p(vloo_particle_t* self, 
	   float u, float v, float w, float s, double birth_ms)
{
    self->super.x = u;
    self->super.y = v;
...
static 
void update_p_v(vloo_particle_t* self, velocity_t* instant_vel)
{
...
{
    object_t super;
    int (*new)(struct vloo_singularity_t* self,
	       float x, float y, float z, double strength, float rate, 
	       double birthdate);
    void (*generate)(struct vloo_singularity_t* self, double time);

    void (*update)(struct vloo_singularity_t* self, double ms);

    velocity_t* (*get_velocity)(struct vloo_singularity_t* self, 
...
static 
int new_s(vloo_singularity_t* self,
	  float x, float y, float z, double strength,
	  float rate, double birthdate)
{
    self->x = x;
    self->y = y;
    self->z = z;
    ...
    {int i; for(i=0; i<MAX_PARTS_PER_SING; i++) {
...
static
void v_s_generate(vloo_singularity_t* self, double time)
{
    double radius = 0.06;
	...
	u *= rr;
	v *= rr;
	p->set(p, self->x + u, self->y + v, variate(self->z, 0.15), 1.0f, time);	
    } }
#endif
...
static
void v_s_update(vloo_singularity_t* self, double ms)
{
...
static
velocity_t* v_s_get_velocity(vloo_singularity_t* self, 
			     float x, float y, float z)
{
...
static
float flare_gen(unsigned int d2, unsigned int of)
{
    float val = 1.0f;
    float d = sqrt(d2/4);
...
static
int vloo_new(effect_t* self)
{
    int ret = 1;
    vloo_t* vloo = (vloo_t*) self;

    ret = ret && vloo->ps->new(vloo->ps, MAX_SINGS*MAX_PARTS_PER_SING,
			       32, flare_gen, 226, 212, 195);
...
static
int vloo_destroy(effect_t* self)
{
    int ret = 1;
    vloo_t* vloo = (vloo_t*) self;
...
static
vloo_singularity_t* vloo_add_singularity(vloo_t* vloo, double now)
{
...
static
void vloo_computes(effect_t* self, void* content, double ms)
{
    vloo_t* vloo = (vloo_t*) self;
    ...
    { int is; for(is=0; is<vloo->singularity_number; is++) {
	...
	{ int ip; for(ip=0; ip<MAX_PARTS_PER_SING; ip++) {
	    ...
	    vloo->ps->particles[ps_i].x = (p->x - 0.5f) / 
		p->z * vloo->super.width / 2 + vloo->super.width / 2;
	    vloo->ps->particles[ps_i].y = (p->y - 0.5f) / 
		p->z * vloo->super.height / 2 + vloo->super.height / 2;
	    vloo->ps->particles[ps_i].size = 5.0 / p->z;
	    vloo->ps->particles[ps_i].size_phase = vloo->ps->opacity_r 
		/ vloo->ps->particles[ps_i].size;
	    ps_i++;
    ...
    vloo->ps->render(vloo->ps, pixels, 
		     vloo->super.width, vloo->super.height, vloo->super.pitch);
}
