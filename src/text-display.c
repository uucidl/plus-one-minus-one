...
static
void text_display_set_text(text_display_t* self, string_t* string)
{
    if(string) {
	self->line_generator = string->get_substring_generator(string, '\n', NULL);
	self->current_line   = NULL;
    }
}

static
void text_display_next_line(text_display_t* self, double start_ms, double end_ms)
{
...
static
void text_display_computes(effect_t* e, void* content, double ms)
{
	...
	if((word_generator =
	    self->current_line->get_substring_generator
	    (self->current_line, ' ', NULL))) {
	    string_t* word;
	    int i = 0;
	    static int selekt = -1;
		...
		if(self->ft.pen_x + word->get_length(word)*self->font_height/2 > self->super.width - self->x) {
		...
		if(selekt == i && ms >= selekt_ms) {
			...
			self->ft.font_height =
			    self->font_height +
			    (selekt_fh-self->font_height)*(1.0 - (ms-selekt_ms)/100.0);
		}
