...
void UpdateSettings(bool updateBasicConfig)
{
    ...
    } else // modplug seems to ignore the SetWaveConfigEx() setting for bass
           // boost
    ...
    if (updateBasicConfig) {
        CSoundFile::SetWaveConfig(gSettings.mFrequency, gSettings.mBits,
                                  gSettings.mChannels);
    ...
    CSoundFile::SetWaveConfigEx(
        gSettings.mFlags & MODPLUG_ENABLE_SURROUND,
        !(gSettings.mFlags & MODPLUG_ENABLE_OVERSAMPLING),
        gSettings.mFlags & MODPLUG_ENABLE_REVERB, true,
        gSettings.mFlags & MODPLUG_ENABLE_MEGABASS,
        gSettings.mFlags & MODPLUG_ENABLE_NOISE_REDUCTION, false);
...
{
    if (tick_duration) {
        *tick_duration = 5000.0 * file->mSoundFile.m_nTempoFactor /
