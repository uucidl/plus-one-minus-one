...
//--------------------------------------------------------------
{
    ...
    WORD norders = 0, restartpos = 0, channels = 0, patterns = 0,
         instruments = 0;
    ...
    if ((!lpStream) || (dwMemLength < 0x200))
        return FALSE;
    if (strnicmp((LPCSTR)lpStream, "Extended Module", 15))
        return FALSE;
    ...
    for (UINT ipat = 0; ipat < patterns; ipat++) {
        ...
        if (ipatmap < MAX_PATTERNS) {
            ...
            if ((Patterns[ipatmap] = AllocatePattern(rows, m_nChannels)) ==
                NULL)
                return TRUE;
            if (!packsize)
                continue;
    ...
    for (UINT iIns = 1; iIns <= instruments; iIns++) {
        ...
        if (dwMemPos + bswapLE32(pih->size) > dwMemLength)
            return TRUE;
        if ((Headers[iIns] = new INSTRUMENTHEADER) == NULL)
            continue;
        ...
        for (UINT ins = 0; ins < nsamples; ins++) {
            ...
            if (xmss.type & 0x20)
                flags[ins] = (xmss.type & 0x10) ? RS_STPCM16D : RS_STPCM8D;
            ...
            pins->nLength = (xmss.samplen > MAX_SAMPLE_LENGTH)
                                ? MAX_SAMPLE_LENGTH
                                : xmss.samplen;
        ...
        for (UINT ismpd = 0; ismpd < nsamples; ismpd++) {
            if ((samplemap[ismpd]) && (samplesize[ismpd]) &&
                ...
                ReadSample(&Ins[samplemap[ismpd]], flags[ismpd],
                           (LPSTR)(lpStream + dwMemPos),
...
//---------------------------------------------------------
{
    ...
    for (i = 0; i < MAX_ORDERS; i++) {
        ...
        if ((Order[i] >= header.patterns) && (Order[i] < MAX_PATTERNS))
            header.patterns = Order[i] + 1;
    }
    header.instruments = m_nInstruments;
    if (!header.instruments)
        header.instruments = m_nSamples;
    ...
    for (i = 1; i <= header.instruments; i++) {
        ...
        for (UINT ins = 0; ins < xmih.samples; ins++) {
            ...
            if (smptable[ins])
                memcpy(xmss.name, m_szNames[smptable[ins]], 22);
        ...
        for (UINT ismpd = 0; ismpd < xmih.samples; ismpd++) {
            ...
            if (pins->pSample) {
#ifndef NO_PACKING
                if ((flags[ismpd] == RS_ADPCM4) && (xmih.samples > 1))
                    CanPackSample(pins->pSample, pins->nLength, nPacking);
#endif // NO_PACKING
    ...
    if ((m_nPatternNames) && (m_lpszPatternNames)) {
        ...
        while ((dwLen >= MAX_PATTERNNAME) &&
               (!m_lpszPatternNames[dwLen - MAX_PATTERNNAME]))
