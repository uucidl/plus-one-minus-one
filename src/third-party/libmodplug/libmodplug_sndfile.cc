...
extern void AMSUnpack(const char *psrc, UINT inputlen, char *pdest, UINT dmax,
                      char packcharacter);
...
//------------------------
{
    ...
    for (i = 0; i < MAX_MIXPLUGINS; i++) {
        if ((m_MixPlugins[i].nPluginDataSize) &&
            (m_MixPlugins[i].pPluginData)) {
...
//----------------------------------------------------------------------------
{
    ...
    if (bMMX)
        d |= SNDMIX_ENABLEMMX;
    if ((gdwMixingFreq != nRate) || (gnBitsPerSample != nBits) ||
        (gnChannels != nChannels) || (d != gdwSoundSetup))
        bReset = TRUE;
...
//--------------------------------------------
{
    DWORD d =
        gdwSoundSetup &
        ~(SNDMIX_NORESAMPLING | SNDMIX_HQRESAMPLER | SNDMIX_ULTRAHQSRCMODE);
    switch (nMode) {
...
//----------------------------------------------------------
{
    ...
    if ((nVol < m_nMasterVolume) && (nVol) && (gdwSoundSetup & SNDMIX_AGC) &&
        (bAdjustAGC)) {
...
//---------------------------------------
{
    ...
    m_dwSongFlags &= ~(SONG_PATTERNLOOP | SONG_CPUVERYHIGH | SONG_FADINGSONG |
    ...
    // Buggy position ?
    if ((nPattern >= MAX_ORDERS) || (Order[nPattern] >= MAX_PATTERNS) ||
...
//------------------------------
{
    m_dwSongFlags &= ~(SONG_CPUVERYHIGH | SONG_FADINGSONG | SONG_ENDREACHED |
                       SONG_GLOBALFADE);
    m_nBufferCount = 0;
...
//----------------------------------------
{
    if ((!m_nSamples) || (!m_nChannels))
        return MOD_TYPE_NONE;
    if (!m_nType)
        return MOD_TYPE_NONE;
    if (m_nType & (MOD_TYPE_MOD | MOD_TYPE_OKT))
        return MOD_TYPE_MOD;
    if (m_nType & (MOD_TYPE_S3M | MOD_TYPE_STM | MOD_TYPE_ULT | MOD_TYPE_FAR |
                   MOD_TYPE_PTM))
        return MOD_TYPE_S3M;
...
{
    UINT n = 0;
    if ((!m_nSamples) || (!m_nChannels) || (m_nType == MOD_TYPE_NONE))
        return 0;
    switch (m_nType) {
    case MOD_TYPE_MOD:
        n = MOD_TYPE_MOD;
...
//------------------------------------------------
{
    ...
    if (delta >= 0) {
        for (i = 0; i < 7; i++)
            if (delta <= (int)CompressionTable[i + 1])
                break;
    } else {
        for (i = 8; i < 15; i++)
            if (delta >= (int)CompressionTable[i + 1])
...
BOOL CSoundFile::CanPackSample(LPSTR pSample, UINT nLen, UINT nPacking,
                               BYTE *result)
//-----------------------------------------------------------------------------------
{
...
UINT CSoundFile::WriteSample(FILE *f, MODINSTRUMENT *pins, UINT nFlags,
                             UINT nMaxLen)
//-----------------------------------------------------------------------------------
{
    UINT len = 0, bufcount;
...
//------------------------------------------------------------------------------------------------
{
    ...
    case RS_IT21516:
        ...
        if ((nFlags == RS_IT2148) || (nFlags == RS_IT2158))
            ITUnpack8Bit(pIns->pSample, pIns->nLength, (LPBYTE)lpMemFile,
                         dwMemLength, (nFlags == RS_IT2158));
        else
            ITUnpack16Bit(pIns->pSample, pIns->nLength, (LPBYTE)lpMemFile,
                          dwMemLength, (nFlags == RS_IT21516));
        break;
    ...
    case RS_MDL16:
        ...
        if (len >= 4) {
            ...
            for (UINT j = 0; j < pIns->nLength; j++) {
                ...
                BYTE sign;
                if (nFlags == RS_MDL16)
                    lowbyte = (BYTE)MDLReadBits(bitbuf, bitnum, ibuf, 8);
    ...
    case RS_DMF16:
        ...
        if (len >= 4) {
            ...
            if (pIns->uFlags & CHN_16BIT)
                maxlen <<= 1;
            LPBYTE ibuf = (LPBYTE)lpMemFile,
                   ibufmax = (LPBYTE)(lpMemFile + dwMemLength);
    ...
    case RS_PCM32S:
        ...
        if (len > 4 * 8) {
            ...
            for (UINT j = 0; j < len; j += slsize) {
                LONG l = ((((pSrc[j + 2] << 8) + pSrc[j + 1]) << 8) + pSrc[j])
                         << 8;
                l /= 256;
                if (l > max)
                    max = l;
            ...
            for (UINT k = 0; k < len; k += slsize) {
                LONG l = ((((pSrc[k + 2] << 8) + pSrc[k + 1]) << 8) + pSrc[k])
                         << 8;
    ...
    case RS_STIPCM32S:
        ...
        if (len > 8 * 8) {
            ...
            signed short *pDest = (signed short *)pIns->pSample;
            for (UINT k = 0; k < len; k += slsize) {
                LONG lr = ((((pSrc[k + 2] << 8) + pSrc[k + 1]) << 8) + pSrc[k])
                          << 8;
                k += slsize;
                LONG ll = ((((pSrc[k + 2] << 8) + pSrc[k + 1]) << 8) + pSrc[k])
                          << 8;
    ...
    case RS_STIPCM16M: {
        ...
        for (UINT j = 0; j < len; j++) {
            ...
            pSample[j * 2 + 1] =
                (signed short)(((UINT)psrc[2] << 8) | (psrc[3]));
            psrc += 4;
        }
        len *= 4;
    } break;
...
//----------------------------------------------------
{
    ...
    if (pIns->uFlags & CHN_16BIT) {
        ...
        if (pIns->uFlags & CHN_STEREO) {
            pSample[len * 2 + 6] = pSample[len * 2 + 4] = pSample[len * 2 + 2] =
                pSample[len * 2] = pSample[len * 2 - 2];
            pSample[len * 2 + 7] = pSample[len * 2 + 5] = pSample[len * 2 + 3] =
                pSample[len * 2 + 1] = pSample[len * 2 - 1];
        } else {
...
//-----------------------------------------------------------
{
    ...
    _asm
    {
        fild transp fld _factor fmulp st(1),
            st(0) fist result fisub result f2xm1 fild result fld _fbase fscale
                fstp st(1) fmul st(1),
            st(0) faddp st(1), st(0) fistp freq
    }
...
//----------------------------------------------
{
    ...
    _asm
    {
        fld _factor fild freq fld _f1_8363 fmulp st(1), st(0) fyl2x fistp result
    }
    return result;
#endif
}
...
//---------------------------------------------------------
{
    ...
    memcpy(m_lpszPatternNames + nPat * MAX_PATTERNNAME, szName,
           MAX_PATTERNNAME);
    return TRUE;
}
...
//-----------------------------------------------
{
    ...
    if (m_nInstruments) {
        ...
        for (UINT ichk = 1; ichk <= m_nSamples; ichk++) {
            if ((!pbIns[ichk]) && (Ins[ichk].pSample))
                nExt++;
        }
    }
    return nExt;
...
void CSoundFile::SetRowCallback(row_callback_f cb, void *closure)
{
    this->row_callback = cb;
    this->row_callback_closure = closure;
}

void CSoundFile::SetEndOfModuleCallback(eom_callback_f cb, void *closure)
{
    this->end_of_module_callback = cb;
    this->end_of_module_callback_closure = closure;
}
