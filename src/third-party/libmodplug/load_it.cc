...
BOOL CSoundFile::ITInstrToMPT(const void *p, INSTRUMENTHEADER *penv,
                              UINT trkvers)
//--------------------------------------------------------------------------------
{
    if (trkvers < 0x0200) {
        ...
        for (UINT ev = 0; ev < 25; ev++) {
            ...
            penv->VolPoints[ev] = (pis->volenv.data[ev * 3 + 2] << 8) |
                                  (pis->volenv.data[ev * 3 + 1]);
            penv->PanEnv[ev] = pis->panenv.data[ev * 3] + 32;
            penv->PanPoints[ev] = (pis->panenv.data[ev * 3 + 2] << 8) |
                                  (pis->panenv.data[ev * 3 + 1]);
            penv->PitchEnv[ev] = pis->pitchenv.data[ev * 3] + 32;
            penv->PitchPoints[ev] = (pis->pitchenv.data[ev * 3 + 2] << 8) |
                                    (pis->pitchenv.data[ev * 3 + 1]);
        }
        penv->nNNA = pis->nna;
        penv->nDCT = pis->dct;
        penv->nDNA = pis->dca;
...
//--------------------------------------------------------------
{
    ...
    if (pifh.globalvol) {
        ...
        if (!m_nDefaultGlobalVolume)
            m_nDefaultGlobalVolume = 256;
        if (m_nDefaultGlobalVolume > 256)
            m_nDefaultGlobalVolume = 256;
    }
    if (pifh.speed)
        m_nDefaultSpeed = pifh.speed;
    if (pifh.tempo)
        m_nDefaultTempo = pifh.tempo;
    ...
    for (int ipan = 0; ipan < 64; ipan++)
        if (pifh.chnpan[ipan] != 0xFF) {
            ...
            if (pifh.chnpan[ipan] & 0x80)
                ChnSettings[ipan].dwFlags |= CHN_MUTE;
            UINT n = pifh.chnpan[ipan] & 0x7F;
            if (n <= 64)
                ChnSettings[ipan].nPan = n << 2;
            if (n == 100)
                ChnSettings[ipan].dwFlags |= CHN_SURROUND;
        }
    if (m_nChannels < 4)
        m_nChannels = 4;
    ...
    if ((pifh.special & 0x01) && (pifh.msglength) &&
        ...
        if (m_lpszSongComments) {
            memcpy(m_lpszSongComments, lpStream + pifh.msgoffset,
                   pifh.msglength);
    ...
    if (inspossize > MAX_INSTRUMENTS)
        inspossize = MAX_INSTRUMENTS;
    inspossize <<= 2;
    ...
    if (smppossize > MAX_SAMPLES)
        smppossize = MAX_SAMPLES;
    smppossize <<= 2;
    ...
    if (patpossize > MAX_PATTERNS)
        patpossize = MAX_PATTERNS;
    patpossize <<= 2;
    ...
    if ((dwMemPos + 8 < dwMemLength) &&
        (bswapLE32(*((DWORD *)(lpStream + dwMemPos))) == 0x4d414e50)) {
    ...
    if ((dwMemPos + 8 < dwMemLength) &&
        (bswapLE32(*((DWORD *)(lpStream + dwMemPos))) == 0x4d414e43)) {
        UINT len = bswapLE32(*((DWORD *)(lpStream + dwMemPos + 4)));
        dwMemPos += 8;
    ...
    if (m_nInstruments >= MAX_INSTRUMENTS)
        m_nInstruments = MAX_INSTRUMENTS - 1;
        ...
        if ((smppos[nsmp]) &&
            ...
            if (pis.id == 0x53504D49) {
                ...
                if ((pis.samplepointer) && (pis.samplepointer < dwMemLength) &&
                    ...
                    ReadSample(&Ins[nsmp + 1], flags,
                               (LPSTR)(lpStream + pis.samplepointer),
    ...
    for (UINT npat = 0; npat < npatterns; npat++) {
        ...
        if ((rows < 4) || (rows > 256))
            continue;
        if (patpos[npat] + 8 + len > dwMemLength)
            continue;
        PatternSize[npat] = rows;
        if ((Patterns[npat] = AllocatePattern(rows, m_nChannels)) == NULL)
            continue;
...
//---------------------------------------------------------
{
    ...
    header.ordnum = 0;
    while ((header.ordnum < MAX_ORDERS) && (Order[header.ordnum] < 0xFF))
        header.ordnum++;
    if (header.ordnum < MAX_ORDERS)
        Order[header.ordnum++] = 0xFF;
    ...
    for (UINT ich = 0; ich < m_nChannels; ich++) {
        ...
        if (ChnSettings[ich].dwFlags & CHN_SURROUND)
            header.chnpan[ich] = 100;
        header.chnvol[ich] = ChnSettings[ich].nVolume;
        if (ChnSettings[ich].dwFlags & CHN_MUTE)
            header.chnpan[ich] |= 0x80;
    ...
    if ((m_nPatternNames) && (m_lpszPatternNames)) {
        ...
        while ((dwPatNamLen >= MAX_PATTERNNAME) &&
               (!m_lpszPatternNames[dwPatNamLen - MAX_PATTERNNAME]))
    ...
    if (m_lpszSongComments) {
        ...
        header.msgoffset = dwHdrPos + dwExtra + header.insnum * 4 +
                           header.patnum * 4 + header.smpnum * 4;
    }
    // Write file header
    ...
    dwPos = dwHdrPos + dwExtra +
            (header.insnum + header.smpnum + header.patnum) * 4;
    if (header.special & 1) {
    ...
    for (UINT nins = 1; nins <= header.insnum; nins++) {
        ...
        if (Headers[nins]) {
            ...
            for (UINT i = 0; i < 120; i++)
                if (penv->Keyboard[i] < MAX_SAMPLES) {
    ...
    for (UINT npat = 0; npat < header.patnum; npat++) {
        ...
        if (PatternSize[npat] == 64) {
            ...
            for (UINT iz = 0; iz < nz; iz++) {
                if ((pzc[iz].note) || (pzc[iz].instr) || (pzc[iz].volcmd) ||
                    (pzc[iz].command))
                    break;
            }
            if (iz == nz) {
    ...
    for (UINT nsmp = 1; nsmp <= header.smpnum; nsmp++) {
        ...
        if (m_nInstruments) {
            for (UINT iu = 1; iu <= m_nInstruments; iu++)
                if (Headers[iu]) {
                    INSTRUMENTHEADER *penv = Headers[iu];
                    for (UINT ju = 0; ju < 128; ju++)
                        if (penv->Keyboard[ju] == nsmp) {
                            itss.flags = 0x01;
                            break;
                        }
                }
...
void ITUnpack8Bit(signed char *pSample, DWORD dwLen, LPBYTE lpMemFile,
    ...
    while (dwLen) {
        ...
        do {
            ...
            if (bLeft < 7) {
                ...
                bLeft = ((BYTE)wBits < bLeft) ? (BYTE)wBits
                                              : (BYTE)((wBits + 1) & 0xFF);
                goto Next;
            }
            if (bLeft < 9) {
...
void ITUnpack16Bit(signed char *pSample, DWORD dwLen, LPBYTE lpMemFile,
                   DWORD dwMemLength, BOOL b215)
//--------------------------------------------------------------------------------------------
{
...
//----------------------------------------------------
{
    ...
    for (UINT i = 0; i < MAX_MIXPLUGINS; i++) {
        ...
        if ((p->Info.dwPluginId1) || (p->Info.dwPluginId2)) {
            nPluginSize =
                sizeof(SNDMIXPLUGININFO) + 4; // plugininfo+4 (datalen)
            ...
            if (f) {
                ...
                if (m_MixPlugins[i].pPluginData) {
                    fwrite(m_MixPlugins[i].pPluginData, 1,
                           m_MixPlugins[i].nPluginDataSize, f);
                }
            }
    ...
    while (nPos + 8 < nLen) {
        ...
        if ((bswapLE32(*(DWORD *)(p + nPos))) == 0x58464843) {
            for (UINT ch = 0; ch < 64; ch++)
                if (ch * 4 < nPluginSize) {
                    ChnSettings[ch].nMixPlugin =
                        bswapLE32(*(DWORD *)(p + nPos + 8 + ch * 4));
                }
        } else {
            ...
            if ((nPlugin < MAX_MIXPLUGINS) &&
                ...
                DWORD dwExtra = bswapLE32(
                    *(DWORD *)(p + nPos + 8 + sizeof(SNDMIXPLUGININFO)));
                m_MixPlugins[nPlugin].Info =
                    *(const SNDMIXPLUGININFO *)(p + nPos + 8);
                m_MixPlugins[nPlugin].Info.dwPluginId1 =
                    bswapLE32(m_MixPlugins[nPlugin].Info.dwPluginId1);
                m_MixPlugins[nPlugin].Info.dwPluginId2 =
                    bswapLE32(m_MixPlugins[nPlugin].Info.dwPluginId2);
                m_MixPlugins[nPlugin].Info.dwInputRouting =
                    bswapLE32(m_MixPlugins[nPlugin].Info.dwInputRouting);
                m_MixPlugins[nPlugin].Info.dwOutputRouting =
                    bswapLE32(m_MixPlugins[nPlugin].Info.dwOutputRouting);
                for (UINT j = 0; j < 4; j++) {
                    m_MixPlugins[nPlugin].Info.dwReserved[j] =
                        bswapLE32(m_MixPlugins[nPlugin].Info.dwReserved[j]);
                }
                if ((dwExtra) &&
                    (dwExtra <= nPluginSize - sizeof(SNDMIXPLUGININFO) - 4)) {
                    m_MixPlugins[nPlugin].nPluginDataSize = 0;
                    m_MixPlugins[nPlugin].pPluginData =
                        new signed char[dwExtra];
                    if (m_MixPlugins[nPlugin].pPluginData) {
                        m_MixPlugins[nPlugin].nPluginDataSize = dwExtra;
                        memcpy(m_MixPlugins[nPlugin].pPluginData,
