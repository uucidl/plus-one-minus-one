...
//-----------------------------------------
{
    ...
    if (gdwSoundSetup & SNDMIX_SURROUND) {
        ...
        if (m_nProLogicDepth < 8)
            nDolbyDepth = (32 >> m_nProLogicDepth) + 32;
        else
            nDolbyDepth =
                (m_nProLogicDepth < 16) ? (8 + (m_nProLogicDepth - 8) * 7) : 64;
        nDolbyDepth >>= 2;
    }
// Reverb Setup
#ifndef NO_REVERB
    if (gdwSoundSetup & SNDMIX_REVERB) {
        ...
        if ((bReset) || (nrs != (UINT)nReverbSize) ||
            (nfa != (UINT)nFilterAttn)) {
            nFilterAttn = nfa;
            nReverbSize = nrs;
...
//------------------------------------------
{
    ...
    if (gdwSoundSetup & SNDMIX_REVERB) {
        ...
        do {
            int echo = ReverbBuffer[nReverbBufferPos] +
                       ReverbBuffer2[nReverbBufferPos2] +
                       ReverbBuffer3[nReverbBufferPos3] +
                       ReverbBuffer4[nReverbBufferPos4]; // echo = reverb signal
            // Delay line and remove Low Frequencies			// v =
            // original
            // signal
            int echodly = ReverbLoFilterDelay[nReverbLoDlyPos]; // echodly =
                                                                // delayed
                                                                // signal
...
BOOL CSoundFile::SetWaveConfigEx(BOOL bSurround, BOOL bNoOverSampling,
                                 BOOL bReverb, BOOL hqido, BOOL bMegaBass,
                                 BOOL bNR, BOOL bEQ)
//----------------------------------------------------------------------------------------------------------------------------
{
    DWORD d = gdwSoundSetup &
              ~(SNDMIX_SURROUND | SNDMIX_NORESAMPLING | SNDMIX_REVERB |
                SNDMIX_HQRESAMPLER | SNDMIX_MEGABASS | SNDMIX_NOISEREDUCTION |
                SNDMIX_EQ);
    if (bSurround)
        d |= SNDMIX_SURROUND;
    if (bNoOverSampling)
