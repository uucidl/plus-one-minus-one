...
//-----------------------------------------------------------
{
    ...
    if ((!lpStream) || (dwMemLength < sizeof(MT2FILEHEADER)) ||
        (pfh->dwMT20 != 0x3032544D) || (pfh->wVersion < 0x0200) ||
        (pfh->wVersion >= 0x0300) || (pfh->wChannels < 4) ||
        (pfh->wChannels > 64))
        return FALSE;
    pdd = NULL;
            ...
            for (UINT iEnv = 0; iEnv < 14; iEnv++) {
                if (pma->dwFlags & (1 << iEnv)) {
                    ...
                    Log("  Env[%d/%d] %04X @%04X: %d points\n", iAuto,
                        nAutoCount, 1 << iEnv, dwMemPos - 8, nPoints);
#endif
                    dwMemPos += 260;
                }
            }
...
// Load Instruments
    ...
    m_nInstruments = (pfh->wInstruments < MAX_INSTRUMENTS)
                         ? pfh->wInstruments
                         : MAX_INSTRUMENTS - 1;
                ...
                for (UINT iEnv = 0; iEnv < 4; iEnv++)
                    if (pehdr[iEnv]) {
                        ...
                        Log("  Env %d.%d @%04X: %d points\n", iIns, iEnv,
                            (UINT)(((BYTE *)pme) - lpStream), pme->nPoints);
#endif
                        switch (iEnv) {
                        // Volume Envelope
                        case 0:
                        ...
                        default:
                            ...
                            penv->nPitchEnv =
                                (pme->nPoints > 16) ? 16 : pme->nPoints;
                            penv->nPitchSustainBegin = penv->nPitchSustainEnd =
                                pme->nSustainPos;
    ...
    for (UINT iSmp = 1; iSmp <= 256; iSmp++) {
        ...
        if (pms->dwDataLen > 0) {
            ...
            if (iSmp < MAX_SAMPLES) {
                ...
                psmp->nC4Speed =
                    TransposeToFrequency(psmp->RelativeTone, psmp->nFineTune);
                if (pms->nQuality == 2) {
        ...
        if ((iData < m_nSamples) && (SampleMap[iData])) {
            ...
            if (!(pms->nFlags & 5)) {
                if (psmp->nLength > 0) {
                    ...
                    if (pms->nChannels == 2)
                        rsflags = (psmp->uFlags & CHN_16BIT) ? RS_STPCM16D
                                                             : RS_STPCM8D;
                    else
                        rsflags =
                    ...
                    dwMemPos +=
                        ReadSample(psmp, rsflags, (LPCSTR)(lpStream + dwMemPos),
