...
static float pow(float a, float b)
{
    ...
    _asm
    {
        fld b            // Load b
            ...
            fmul ST(1),
            ST(0) // multiply with fractional part
            faddp ST(1),
            ST(0)       // add integer_part
...
//-----------------------------------------------------------------------
{
    ...
    if (m_dwSongFlags & SONG_EXFILTERRANGE)
        Fc = 110.0f * pow(2.0f, 0.25f +
                                    ((float)(nCutOff * (flt_modifier + 256))) /
                                        (21.0f * 512.0f));
    else
        Fc = 110.0f * pow(2.0f, 0.25f +
                                    ((float)(nCutOff * (flt_modifier + 256))) /
    ...
    if (freq > 10000)
        return 10000;
    if (freq * 2 > (LONG)gdwMixingFreq)
        freq = gdwMixingFreq >> 1;
    return (DWORD)freq;
}

// Simple 2-poles resonant filter
void CSoundFile::SetupChannelFilter(MODCHANNEL *pChn, BOOL bReset,
                                    int flt_modifier) const
//----------------------------------------------------------------------------------------
{
