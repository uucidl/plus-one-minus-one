 ...
 * mix very
 * well.  Some mods will be missing instruments for a short time after a seek,
 * as ModPlug
 ...
 * set a row callback
 */
void ModPlug_SetRowCallback(ModPlugFile *file, row_callback_f cb,
                            void *closure);

/*
 * set an end of module callback
 */
void ModPlug_SetEndOfModuleCallback(ModPlugFile *file, eom_callback_f cb,
                                    void *closure);

/*
 * set the repeat count. (-1 for infinite, 0 for none, n for bounded)
 */
 ...
 */
void ModPlug_GetMusicSpeed(ModPlugFile *file, double *tick_duration,
...
typedef struct _ModPlug_Settings {
    int mFlags; /* One or more of the MODPLUG_ENABLE_* flags above,
                   bitwise-OR'ed */
