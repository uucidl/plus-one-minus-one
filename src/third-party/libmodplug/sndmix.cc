	...
	if (++m_nTickCount >= m_nMusicSpeed * (m_nPatternDelay+1) + m_nFrameDelay)
	{
		...
		if (!(m_dwSongFlags & SONG_PATTERNLOOP))
		{
			...
			while (m_nPattern >= MAX_PATTERNS)
			{
				...
				if ((m_nPattern == 0xFF) || (m_nCurrentPattern >= MAX_ORDERS))
				{
					...
					/* correct only if the module is started at the very beginning */
					double ms = 5000.0 * m_nTempoFactor * m_nTotalCount / (m_nMusicTempo << 8); 
					end_of_module_callback(this->end_of_module_callback_closure, ms);
				    }
				        if (!m_nRepeatCount)
		...
		for (UINT nChn=0; nChn<m_nChannels; pChn++, nChn++, m++)
		{
			...
			if(this->row_callback) {
			    ...
			    //long unsigned int smps = (gdwMixingFreq * 5 * m_nTempoFactor) / (m_nMusicTempo << 8) * m_nTotalCount;
			    double ms = 5000.0 * m_nTempoFactor * m_nTotalCount / (m_nMusicTempo << 8); /* correct only if the module is started at the very beginning */
			    row_callback(this->row_callback_closure, 
					 ms, nChn, m->note, 
					 m->instr, m->volcmd, 
					 vol, m->command, m->param);
			}
