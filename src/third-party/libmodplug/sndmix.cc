...
const UINT PreAmpAGCTable[16] = {
    0x60, 0x60, 0x60, 0x60, 0x68, 0x70, 0x78, 0x80,
    ...
    _asm
    {
        mov eax, a mov ebx, b or eax, eax mov edx, eax jge aneg neg eax aneg
            : xor edx,
              ebx or ebx,
              ebx mov ecx,
              c jge bneg neg ebx bneg
              : xor edx,
                ecx or ecx,
                ecx mov sign,
                edx jge cneg neg ecx cneg
                : mul ebx cmp edx,
                  ecx jae diverr div ecx jmp ok diverr
                  : mov eax,
                    0x7fffffff ok : mov edx,
                                    sign or edx,
                                    edx jge rneg neg eax rneg : mov result,
                                                                eax
    }
    return result;
#else
...
//--------------------------------------
{
    ...
    if (gdwMixingFreq < 4000)
        gdwMixingFreq = 4000;
    if (gdwMixingFreq > MAX_SAMPLE_RATE)
        gdwMixingFreq = MAX_SAMPLE_RATE;
...
//-------------------------------------------------------
{
...

    {
        ...
        while ((nMaxPlugins > 0) && (!m_MixPlugins[nMaxPlugins - 1].pMixPlugin))
            nMaxPlugins--;
    }
    m_nMixStat = 0;
    ...
    while (lRead > 0) {
        ...
        X86_StereoFill(MixSoundBuffer, lSampleCount, &gnDryROfsVol,
                       &gnDryLOfsVol);
        if (gnChannels >= 2) {
            lSampleCount *= 2;
        ...
        // Multichannel
        if (gnChannels > 2) {
            X86_InterleaveFrontRear(MixSoundBuffer, MixRearBuffer,
                                    lSampleCount);
            lTotalSampleCount *= 2;
        }
        // Hook Function
...
MixDone:
    if (lRead)
        memset(lpBuffer, (gnBitsPerSample == 8) ? 0x80 : 0,
               lRead * lSampleSize);
    // VU-Meter
    ...
    if ((gnVUMeter = (UINT)(nVUMeterMax - nVUMeterMin)) > 0xFF)
        gnVUMeter = 0xFF;
    if (nStat) {
        m_nMixStat += nStat - 1;
        m_nMixStat /= nStat;
...
{
    if (++m_nTickCount >=
        m_nMusicSpeed * (m_nPatternDelay + 1) + m_nFrameDelay) {
        m_nPatternDelay = 0;
        m_nFrameDelay = 0;
        m_nTickCount = 0;
        m_nRow = m_nNextRow;
        ...
        if (!(m_dwSongFlags & SONG_PATTERNLOOP)) {
            m_nPattern = (m_nCurrentPattern < MAX_ORDERS)
                             ? Order[m_nCurrentPattern]
                             : 0xFF;
            ...
            while (m_nPattern >= MAX_PATTERNS) {
                ...
                if ((m_nPattern == 0xFF) || (m_nCurrentPattern >= MAX_ORDERS)) {
                    // call the end of module callback if set
                    if (this->end_of_module_callback) {
                        /* correct only if the module is started at the very
                         * beginning */
                        double ms = 5000.0 * m_nTempoFactor * m_nTotalCount /
                                    (m_nMusicTempo << 8);
                        end_of_module_callback(
                            this->end_of_module_callback_closure, ms);
                    }
                    if (!m_nRepeatCount)
        ...
        for (UINT nChn = 0; nChn < m_nChannels; pChn++, nChn++, m++) {
            ...
            // call the row callback if set
            if (this->row_callback) {
                unsigned char vol = m->vol ?: Ins[m->instr].nGlobalVol;
                // long unsigned int smps = (gdwMixingFreq * 5 * m_nTempoFactor)
                // / (m_nMusicTempo << 8) * m_nTotalCount;
                double ms =
    ...
    if (m_nTickCount) {
        ...
        if ((!(m_nType & MOD_TYPE_XM)) &&
            (m_nTickCount < m_nMusicSpeed * (1 + m_nPatternDelay))) {
...
//-------------------------
{
    ...
    DWORD nMasterVol;
    {
        ...
        if ((m_nType & MOD_TYPE_IT) && (m_nInstruments) && (nchn32 < 6))
            nchn32 = 6;
        int realmastervol = m_nMasterVolume;
        if (realmastervol > 0x80) {
            realmastervol = 0x80 + ((realmastervol - 0x80) * (nchn32 + 4)) / 16;
        }
        UINT attenuation = (gdwSoundSetup & SNDMIX_AGC)
                               ? PreAmpAGCTable[nchn32 >> 1]
    ...
    for (UINT nChn = 0; nChn < MAX_CHANNELS; nChn++, pChn++) {
        ...
        if ((pChn->nPeriod) && (pChn->nLength)) {
            ...
            if (pChn->dwFlags & CHN_TREMOLO) {
                ...
                if (vol > 0) {
                    ...
                    case 1:
                        vol += (ModRampDownTable[trempos] *
                                (int)pChn->nTremoloDepth) >>
                               tremattn;
                        break;
                    case 2:
                        vol += (ModSquareTable[trempos] *
                                (int)pChn->nTremoloDepth) >>
                               tremattn;
                        break;
                    case 3:
                        vol += (ModRandomTable[trempos] *
                                (int)pChn->nTremoloDepth) >>
                               tremattn;
                        break;
                    default:
                        vol += (ModSinusTable[trempos] *
                                (int)pChn->nTremoloDepth) >>
                               tremattn;
                    }
                }
                if ((m_nTickCount) ||
                    ((m_nType & (MOD_TYPE_STM | MOD_TYPE_S3M | MOD_TYPE_IT)) &&
            ...
            if (pChn->pHeader) {
                ...
                if ((penv->nPPS) && (pChn->nRealPan) && (pChn->nNote)) {
                    int pandelta = (int)pChn->nRealPan +
                                   (int)((int)(pChn->nNote - penv->nPPC - 1) *
                                         (int)penv->nPPS) /
                                       (int)8;
                    if (pandelta < 0)
                        pandelta = 0;
                ...
                if (pChn->dwFlags &
                    CHN_NOTEFADE) // 1.41-: CHN_KEYOFF|CHN_NOTEFADE
                {
                    pChn->nFadeOutVol = 0;
                    vol = 0;
                }
            ...
            if (vol) {
                ...
                // -> _muldiv( 14+8, 6+6, 18); => RealVolume: 14-bit result
                // (22+12-20)
                pChn->nRealVolume =
                    _muldiv(vol * m_nGlobalVolume,
            ...
            if ((pChn->dwFlags & (CHN_GLISSANDO | CHN_PORTAMENTO)) ==
                ...
                period = GetPeriodFromNote(GetNoteFromPeriod(period),
            ...
            if (pChn->nCommand == CMD_ARPEGGIO) {
                ...
                case 1:
                    period =
                        GetPeriodFromNote(pChn->nNote + (pChn->nArpeggio >> 4),
                                          pChn->nFineTune, pChn->nC4Speed);
                    break;
                case 2:
                    period = GetPeriodFromNote(pChn->nNote +
                                                   (pChn->nArpeggio & 0x0F),
                                               pChn->nFineTune, pChn->nC4Speed);
                    break;
                }
            ...
            if ((pChn->pHeader) && (pChn->dwFlags & CHN_PITCHENV) &&
                ...
                if ((x2 > x1) && (envpos > x1)) {
                    ...
                    envpitch +=
                        ((envpos - x1) * (envpitchdest - envpitch)) / (x2 - x1);
                }
                if (envpitch < -256)
                    envpitch = -256;
                if (envpitch > 256)
...
#ifndef NO_FILTER
                    SetupChannelFilter(
                        pChn, (pChn->dwFlags & CHN_FILTER) ? FALSE : TRUE,
                        envpitch);
#endif // NO_FILTER
                } else
                // Pitch Envelope
                {
                    ...
                    if (l < 0) {
                        ...
                        if (l > 255)
                            l = 255;
                        period =
                            _muldiv(period, LinearSlideUpTable[l], 0x10000);
                    } else {
                        if (l > 255)
                            l = 255;
                        period =
                            _muldiv(period, LinearSlideDownTable[l], 0x10000);
                    }
                }
            }
            ...
            if (pChn->dwFlags & CHN_VIBRATO) {
                ...
                UINT vdepth = ((m_nType != MOD_TYPE_IT) ||
                               (m_dwSongFlags & SONG_ITOLDEFFECTS))
                                  ? 6
                                  : 7;
                ...
                if ((m_dwSongFlags & SONG_LINEARSLIDES) &&
                    ...
                    if (l < 0) {
                        l = -l;
                        vdelta = _muldiv(period, LinearSlideDownTable[l >> 2],
                                         0x10000) -
                                 period;
                        if (l & 0x03)
                            vdelta +=
                                _muldiv(period,
                                        FineLinearSlideDownTable[l & 0x03],
                                        0x10000) -
                                period;

                    } else {
                        vdelta = _muldiv(period, LinearSlideUpTable[l >> 2],
                                         0x10000) -
                                 period;
                        if (l & 0x03)
                            vdelta += _muldiv(period,
                                              FineLinearSlideUpTable[l & 0x03],
                                              0x10000) -
                                      period;
                    }
                }
            ...
            if ((pChn->pInstrument) && (pChn->pInstrument->nVibDepth)) {
                ...
                if (m_nType & MOD_TYPE_IT) {
                    ...
                    n >>= 2;
                    period = _muldiv(
                        period, df1 + ((df2 - df1) * (n & 0x3F) >> 6), 256);
        ...
        if (pChn->pHeader) {
            ...
            if (pChn->dwFlags & CHN_VOLENV) {
                ...
                if ((penv->dwFlags & ENV_VOLSUSTAIN) &&
                    ...
                    if (pChn->nVolEnvPosition ==
                        (UINT)penv->VolPoints[penv->nVolSustainEnd] + 1)
                        pChn->nVolEnvPosition =
                            penv->VolPoints[penv->nVolSustainBegin];
                } else
                    // End of Envelope ?
                    if (pChn->nVolEnvPosition >
                        penv->VolPoints[penv->nVolEnv - 1]) {
                    if ((m_nType & MOD_TYPE_IT) || (pChn->dwFlags & CHN_KEYOFF))
                        pChn->dwFlags |= CHN_NOTEFADE;
                    pChn->nVolEnvPosition = penv->VolPoints[penv->nVolEnv - 1];
                    if ((!penv->VolEnv[penv->nVolEnv - 1]) &&
                        ((nChn >= m_nChannels) || (m_nType & MOD_TYPE_IT))) {
                        pChn->dwFlags |= CHN_NOTEFADE;
                        pChn->nFadeOutVol = 0;
            ...
            if (pChn->dwFlags & CHN_PANENV) {
                ...
                if ((penv->dwFlags & ENV_PANSUSTAIN) &&
                    (pChn->nPanEnvPosition ==
                     (UINT)penv->PanPoints[penv->nPanSustainEnd] + 1) &&
                    (!(pChn->dwFlags & CHN_KEYOFF))) {
                    // Panning sustained
                    pChn->nPanEnvPosition =
            ...
            if (pChn->dwFlags & CHN_PITCHENV) {
                ...
                if ((penv->dwFlags & ENV_PITCHSUSTAIN) &&
                    (!(pChn->dwFlags & CHN_KEYOFF))) {
                    if (pChn->nPitchEnvPosition ==
                        (UINT)penv->PitchPoints[penv->nPitchSustainEnd] + 1)
                        pChn->nPitchEnvPosition =
        ...
        pChn->pCurrentSample =
            ((pChn->pSample) && (pChn->nLength) && (pChn->nInc)) ? pChn->pSample
                                                                 : NULL;
...
#define kChnMasterVol nMasterVol
            ...
            if ((pChn->dwFlags & CHN_SURROUND) && (gnChannels <= 2))
            ...
            if ((pChn->dwFlags & CHN_VOLUMERAMP) &&
                ...
                LONG nRightDelta = ((pChn->nNewRightVol - pChn->nRightVol)
                                    << VOLUMERAMPPRECISION);
                LONG nLeftDelta = ((pChn->nNewLeftVol - pChn->nLeftVol)
                                   << VOLUMERAMPPRECISION);
#ifndef FASTSOUNDLIB
                if ((gdwSoundSetup & SNDMIX_DIRECTTODISK) ||
                    ...
                    if ((pChn->nRightVol | pChn->nLeftVol) &&
                        ...
                        if (nRampLength < (LONG)gnVolumeRampSamples)
                            nRampLength = gnVolumeRampSamples;
                    }
                }
#endif
            ...
            if (m_nMixChannels >= MAX_CHANNELS)
                break;
        } else {
#ifdef ENABLE_STEREOVU
