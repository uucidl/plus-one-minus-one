...
//-----------------------------------------------------
{
    ...
    case 0x0F:
        command =
            (param <=
             (UINT)((m_nType & (MOD_TYPE_XM | MOD_TYPE_MT2)) ? 0x1F : 0x20))
                ? CMD_SPEED
                : CMD_TEMPO;
...
//---------------------------------------------------------------
{
    ...
    if ((IsMagic(s, "M.K.")) || (IsMagic(s, "M!K!")) || (IsMagic(s, "M&K!")) ||
        (IsMagic(s, "N.T.")))
        m_nChannels = 4;
        ...
        m_nChannels = s[1] - '0' + 10;
             ...
             (s[3] == 'H'))
        m_nChannels = s[1] - '0' + 20;
    else if ((s[0] == '3') && (s[1] >= '0') && (s[1] <= '2') && (s[2] == 'C') &&
             (s[3] == 'H'))
        m_nChannels = s[1] - '0' + 30;
    ...
    if ((m_nSamples == 15) && (dwTotalSampleLen > dwMemLength * 4))
        return FALSE;
    ...
    if ((!norders) || (norders > 0x80)) {
        norders = 0x80;
        while ((norders > 1) && (!Order[norders - 1]))
            norders--;
    }
    nbpbuggy = 0;
    nbpbuggy2 = 0;
    ...
    for (UINT iend = norders; iend < MAX_ORDERS; iend++)
        Order[iend] = 0xFF;
    norders--;
    m_nRestartPos = pMagic->nRestartPos;
    if (m_nRestartPos >= 0x78)
        m_nRestartPos = 0;
    if (m_nRestartPos + 1 >= (UINT)norders)
        m_nRestartPos = 0;
    if (!nbp)
        return FALSE;
    DWORD dwWowTest = dwTotalSampleLen + dwMemPos;
    if ((IsMagic(pMagic->Magic, "M.K.")) &&
        (dwWowTest + nbp * 8 * 256 == dwMemLength))
        m_nChannels = 8;
    if ((nbp != nbpbuggy) &&
        (dwWowTest + nbp * m_nChannels * 256 != dwMemLength)) {
        if (dwWowTest + nbpbuggy * m_nChannels * 256 == dwMemLength)
            nbp = nbpbuggy;
        else
            nErr += 8;
    } else if ((nbpbuggy2 > nbp) &&
               (dwWowTest + nbpbuggy2 * m_nChannels * 256 == dwMemLength)) {
        nbp = nbpbuggy2;
    }
    if ((dwWowTest < 0x600) || (dwWowTest > dwMemLength))
        nErr += 8;
    if ((m_nSamples == 15) && (nErr >= 16))
        return FALSE;
    // Default settings
    ...
    for (UINT ipat = 0; ipat < nbp; ipat++) {
        if (ipat < MAX_PATTERNS) {
            if ((Patterns[ipat] = AllocatePattern(64, m_nChannels)) == NULL)
                break;
            PatternSize[ipat] = 64;
            if (dwMemPos + m_nChannels * 256 >= dwMemLength)
                break;
    ...
    for (UINT ismp = 1; ismp <= m_nSamples; ismp++)
        if (Ins[ismp].nLength) {
            ...
            DWORD dwSize =
                ReadSample(&Ins[ismp], flags, p, dwMemLength - dwMemPos);
            if (dwSize) {
                dwMemPos += dwSize;
                dwErrCheck++;
            }
...
//----------------------------------------------------------
{
    ...
    for (UINT iord = 0; iord < 128; iord++) {
        ...
        if ((Order[iord] < 0x80) && (nbp <= Order[iord]))
            nbp = Order[iord] + 1;
    }
    bTab[0] = norders;
    ...
    // Writing instruments
    for (UINT ismpd = 1; ismpd <= 31; ismpd++)
        if (inslen[ismpd]) {
            ...
            if (!(pins->uFlags & (CHN_16BIT | CHN_STEREO))) {
                if ((nPacking) &&
                    (CanPackSample(pins->pSample, inslen[ismpd], nPacking))) {
                    fwrite("ADPCM", 1, 5, f);
                    flags = RS_ADPCM4;
                }
            }
