...
//---------------------------------------------------------------
{
    ...
    WAVEFORMATHEADER *pfmt =
        (WAVEFORMATHEADER *)(lpStream + sizeof(WAVEFILEHEADER));
    if ((!lpStream) || (dwMemLength < (DWORD)sizeof(WAVEFILEHEADER)))
        return FALSE;
    ...
    if ((dwMemPos + 8 >= dwMemLength) ||
        ...
        (pfmt->channels > 4) || (!pfmt->channels) || (!pfmt->freqHz) ||
        (pfmt->bitspersample & 7) || (pfmt->bitspersample < 8) ||
    ...
    for (UINT nChn = 0; nChn < m_nSamples; nChn++) {
        ...
        if (pfmt->bitspersample >= 16) {
            ...
            signed char *psrc = (signed char *)(lpStream + dwMemPos + 8 +
                                                nChn * slsize + slsize - 2);
...
BOOL IMAADPCMUnpack16(signed short *pdest, UINT nLen, LPBYTE psrc,
                      DWORD dwBytes, UINT pkBlkAlign)
//------------------------------------------------------------------------------------------------
{
    ...
    if ((nLen < 4) || (!pdest) || (!psrc) || (pkBlkAlign < 5) ||
        (pkBlkAlign > dwBytes))
        return FALSE;
    nPos = 0;
