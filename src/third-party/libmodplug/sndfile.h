...
#define RS_STIPCM16M                                                           \
    (RS_PCM16M | 0x40 | RSF_STEREO) // stereo 16-bit signed big endian
// 24-bit signed
...
typedef void (*row_callback_f)(void *closure, double time, unsigned int channel,
                               BYTE note, BYTE instr, BYTE volcmd, BYTE volume,
                               BYTE row_command, BYTE row_param);

typedef void (*eom_callback_f)(void *closure, double time);
...
class IMixPlugin
{
  public:
    ...
    virtual void Process(float *pOutL, float *pOutR,
                         unsigned long nSamples) = 0;
    virtual void Init(unsigned long nFreq, int bReset) = 0;
    virtual void MidiSend(DWORD dwMidiCode) = 0;
    virtual void MidiCommand(UINT nMidiCh, UINT nMidiProg, UINT note,
                             UINT vol) = 0;
};
...
typedef VOID (*LPSNDMIXHOOKPROC)(int *, unsigned long,
                                 unsigned long); // buffer, samples, channels
...
//==============
{
  public: // Static Members
    ...
    void *row_callback_closure;
    eom_callback_f end_of_module_callback;
    void *end_of_module_callback_closure;
...
// Save Functions
    ...
    UINT WriteSample(FILE *f, MODINSTRUMENT *pins, UINT nFlags,
                     UINT nMaxLen = 0);
  ...
  public:
    ...
    static BOOL SetWaveConfigEx(BOOL bSurround, BOOL bNoOverSampling,
                                BOOL bReverb, BOOL hqido, BOOL bMegaBass,
                                BOOL bNR, BOOL bEQ);
    // DSP Effects
  ...
  public:
    ...
    BOOL CanPackSample(LPSTR pSample, UINT nLen, UINT nPacking,
                       BYTE *result = NULL);
    ...
    UINT GetFreqFromPeriod(UINT period, UINT nC4Speed,
                           int nPeriodFrac = 0) const;
    // Misc functions
...
#ifndef NO_FILTER
    DWORD
    CutOffToFrequency(UINT nCutOff,
                      int flt_modifier = 256) const; // [0-255] => [1-10KHz]
#endif

    void SetRowCallback(row_callback_f cb, void *closure);
    void SetEndOfModuleCallback(eom_callback_f cb, void *closure);
...
// inline DWORD BigEndian(DWORD x) { return ((x & 0xFF) << 24) | ((x & 0xFF00)
// << 8) | ((x & 0xFF0000) >> 8) | ((x & 0xFF000000) >> 24); }
// inline WORD BigEndianW(WORD x) { return (WORD)(((x >> 8) & 0xFF) | ((x << 8)
// & 0xFF00)); }
