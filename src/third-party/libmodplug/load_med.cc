...
#define MMDTAG_FX_GROUPNAME                                                    \
    (MMDTAG_PTR | 5) // the Global Effects group shouldn't have name saved!
...
typedef struct tagMMDSAMPLEHEADER {
    ...
    // -1: Synth
    // -2: Hybrid
    // if type indicates packed data, these fields follow, otherwise we go right
    // to the data
    ...
    ULONG rightchlen;   // packed length of right channel in bytes (ONLY PRESENT
                        // IN STEREO SAMPLES)
...
typedef struct tagMMD0SONGHEADER {
    ...
    BYTE flags;  // 0x10: Hex Volumes | 0x20: ST/NT/PT Slides | 0x40: 8 Channels
                 // song
...
typedef struct tagMMD2SONGHEADER {
    ...
    WORD deftempo;          // BPM tempo
    signed char playtransp; // play transpose
    BYTE flags; // 0x1:filteron, 0x2:jumpingon, 0x4:jump8th, 0x8:instr_attached,
                // 0x10:hex_vol, 0x20:PT_slides, 0x40:8ch_conv,0x80:hq slows
                // playing speed
...
// For MMD0 the note information is held in 3 bytes, byte0, byte1, byte2.  For
// reference we
// number the bits in each byte 0..7, where 0 is the low bit.
// The note is held as bits 5..0 of byte0
// The instrument is encoded in 6 bits,  bits 7 and 6 of byte0 and bits 7,6,5,4
// of byte1
...
typedef struct tagMMD1BLOCK {
    WORD numtracks; // Number of tracks, may be > 64, but then that data is
                    // skipped.
...
} MMDCOMMAND; // Last entry has offset == 0xFFFF, cmd_number == 0 and 0
              // extrabytes
...
typedef struct tagMMD0EXP {
    ...
    DWORD jumpmask;
    DWORD rgbtable;
    BYTE channelsplit[4]; // Only used if 8ch_conv (extra channel for every
                          // nonzero entry)
    DWORD n_info;
...
//---------------------------------------------------------------
{
    ...
    if (version < '2') {
        ...
        Log("  playseq    = ");
        for (UINT idbg1 = 0; idbg1 < 16; idbg1++)
        ...
        Log("  tempo2     = %d\n", pmsh->tempo2);
        Log("  trkvol     = ");
        for (UINT idbg2 = 0; idbg2 < 16; idbg2++)
    ...
    for (UINT iSHdr = 0; iSHdr < m_nSamples; iSHdr++) {
        ...
        pins->nLoopEnd =
            pins->nLoopStart + (bswapBE16(pmsh->sample[iSHdr].replen) << 1);
    ...
    if (version < '2') {
        ...
        if ((!nSections) || (!sectiontable) ||
            (sectiontable >= dwMemLength - 2))
            nSections = 1;
        nOrders = 0;
    ...
    if (pmex) {
        ...
        if (smpinfoex) {
            ...
            if ((iinfoptr) && (ientrysz < 256) &&
                ...
                for (UINT i = 0; i < ientries; i++)
                    if (i < m_nSamples) {
                        lstrcpyn(m_szNames[i + 1], psznames + i * ientrysz,
                                 maxnamelen);
                    }
            }
        }
        ...
        if ((trackinfo_ofs) &&
            (trackinfo_ofs + m_nChannels * 4 < dwMemLength)) {
            ...
            for (UINT i = 0; i < m_nChannels; i++) {
                ...
                if (trktagofs) {
                    while (trktagofs + 8 < dwMemLength) {
                        DWORD ntag =
                            bswapBE32(*(DWORD *)(lpStream + trktagofs));
                        if (ntag == MMDTAG_END)
                            break;
                        DWORD tagdata =
                            bswapBE32(*(DWORD *)(lpStream + trktagofs + 4));
                        switch (ntag) {
    ...
    // Reading samples
    if (dwSmplArr > dwMemLength - 4 * m_nSamples)
        return TRUE;
    ...
    for (UINT iBlk = 0; iBlk < wNumBlocks; iBlk++) {
        ...
        if (version == '0') {
            ...
            if (!tracks)
                tracks = m_nChannels;
            if ((Patterns[iBlk] = AllocatePattern(lines, m_nChannels)) == NULL)
                continue;
            ...
            for (UINT y = 0; y < lines; y++) {
                for (UINT x = 0; x < tracks; x++, s += 3)
                    if (x < m_nChannels) {
                        ...
                        p->command = s[1] & 0x0F;
                        p->param = s[2];
                        // if (!iBlk) Log("%02X.%02X.%02X | ", s[0], s[1],
                        // s[2]);
                        MedConvert(p, pmsh);
                        p++;
                    }
            ...
            if ((dwBlockInfo) &&
...
#ifdef MED_LOG
                Log("  BLOCKINFO: blockname=0x%04X namelen=%d pagetable=0x%04X "
                    "&cmdexttable=0x%04X\n",
                    bswapBE32(pbi->blockname), bswapBE32(pbi->blocknamelen),
                    bswapBE32(pbi->pagetable), bswapBE32(pbi->cmdexttable));
#endif
                ...
                if (pbi->cmdexttable) {
                    ...
                    if (cmdexttable < dwMemLength - 4) {
                        cmdexttable =
                            bswapBE32(*(DWORD *)(lpStream + cmdexttable));
                        if ((cmdexttable) &&
