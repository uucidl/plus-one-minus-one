...
void UnpackMDLTrack(MODCOMMAND *pat, UINT nChannels, UINT nRows, UINT nTrack,
                    const BYTE *lpTracks)
//-------------------------------------------------------------------------------------------------
{
    ...
    while ((row < nRows) && (pos < len)) {
        ...
        case 0x03: {
            ...
            if ((cmd.command != CMD_SPEED) && (cmd.command != CMD_TEMPO) &&
...
//---------------------------------------------------------------
{
    ...
    while (dwMemPos + 6 < dwMemLength) {
        ...
        case 0x4150:
            ...
            for (i = 0; i < npatterns; i++) {
                ...
                if (m_nChannels < pmpd->channels)
                    m_nChannels = pmpd->channels;
        ...
        case 0x4949:
            ...
            for (i = 0; i < ninstruments; i++) {
                ...
                if ((nins >= MAX_INSTRUMENTS) || (!nins))
                    break;
                if (m_nInstruments < nins)
                    m_nInstruments = nins;
                if (!Headers[nins]) {
                    UINT note = 12;
                    if ((Headers[nins] = new INSTRUMENTHEADER) == NULL)
                        break;
            ...
            for (j = 1; j <= m_nInstruments; j++)
                if (!Headers[j]) {
                    ...
                    if (Headers[j])
                        memset(Headers[j], 0, sizeof(INSTRUMENTHEADER));
                }
            break;
        // VE: Volume Envelope
        case 0x4556:
        ...
        case 0x5349:
            ...
            for (i = 0; i < nsamples; i++, dwPos += 59) {
                ...
                pins->nLoopEnd =
                    pins->nLoopStart + *((DWORD *)(lpStream + dwPos + 53));
        ...
        case 0x4153:
            ...
            for (i = 1; i <= m_nSamples; i++)
                if ((Ins[i].nLength) && (!Ins[i].pSample) &&
                    ...
                    if (!smpinfo[i]) {
                        dwPos +=
                            ReadSample(pins, flags, (LPSTR)(lpStream + dwPos),
                                       dwMemLength - dwPos);
                    } else {
                        ...
                        if ((dwPos + dwLen <= dwMemLength) && (dwLen > 4)) {
                            flags =
                                (pins->uFlags & CHN_16BIT) ? RS_MDL16 : RS_MDL8;
                            ReadSample(pins, flags, (LPSTR)(lpStream + dwPos),
                                       dwLen);
                        }
                        dwPos += dwLen;
    ...
    if ((dwTrackPos) && (npatterns) && (m_nChannels) && (ntracks)) {
        for (UINT ipat = 0; ipat < npatterns; ipat++) {
            if ((Patterns[ipat] =
                     AllocatePattern(PatternSize[ipat], m_nChannels)) == NULL)
                break;
            for (UINT chn = 0; chn < m_nChannels; chn++)
                if ((patterntracks[ipat * 32 + chn]) &&
                    (patterntracks[ipat * 32 + chn] <= ntracks)) {
                    MODCOMMAND *m = Patterns[ipat] + chn;
                    UnpackMDLTrack(m, m_nChannels, PatternSize[ipat],
                                   patterntracks[ipat * 32 + chn],
                                   lpStream + dwTrackPos);
                }
        }
    }
