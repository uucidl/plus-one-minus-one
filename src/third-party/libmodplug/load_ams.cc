...
//-----------------------------------------------------------
{
    ...
    if ((pfh->verhi != 0x01) || (strncmp(pfh->szHeader, "Extreme", 7)) ||
        ...
        (pfh->samples > MAX_SAMPLES) || (pfh->patterns > MAX_PATTERNS) ||
        (pfh->orders > MAX_ORDERS)) {
    ...
    for (UINT nSmp = 1; nSmp <= m_nSamples;
         nSmp++, dwMemPos += sizeof(AMSSAMPLEHEADER)) {
    ...
    tmp2 = (tmp < 32) ? tmp : 31;
    if (tmp2)
        memcpy(m_szNames[0], lpStream + dwMemPos, tmp2);
    m_szNames[0][tmp2] = 0;
    dwMemPos += tmp;
    // Read sample names
    for (UINT sNam = 1; sNam <= m_nSamples; sNam++) {
        ...
        tmp2 = (tmp < 32) ? tmp : 31;
        if (tmp2)
            memcpy(m_szNames[sNam], lpStream + dwMemPos, tmp2);
        dwMemPos += tmp;
    }
    // Skip Channel names
    ...
    for (UINT pNam = 0; pNam < m_nPatternNames; pNam++) {
        ...
        tmp2 = (tmp < 32) ? tmp : 31;
        if (tmp2)
            memcpy(m_lpszPatternNames + pNam * 32, lpStream + dwMemPos, tmp2);
        dwMemPos += tmp;
    }
    // Read Song Comments
    ...
    for (UINT iSmp = 1; iSmp <= m_nSamples; iSmp++)
        if (Ins[iSmp].nLength) {
            ...
            dwMemPos +=
                ReadSample(&Ins[iSmp], flags, (LPSTR)(lpStream + dwMemPos),
...
//------------------------------------------------------------
{
    ...
    for (UINT nIns = 1; nIns <= m_nInstruments; nIns++) {
        ...
        for (UINT ismpmap = 0; ismpmap < pins->samples; ismpmap++) {
            if ((ismpmap >= 16) || (m_nSamples + 1 >= MAX_SAMPLES))
                break;
            m_nSamples++;
        ...
        // Volume Envelope
        {
            ...
            for (UINT i = 0; i < penv->nVolEnv; i++) {
                ...
                pos +=
                    volenv->info[i * 3] + ((volenv->info[i * 3 + 1] & 1) << 8);
                penv->VolPoints[i] = (WORD)pos;
            }
        }
        penv->nFadeOut =
            (((lpStream[dwMemPos + 2] & 0x0F) << 8) | (lpStream[dwMemPos + 1]))
            << 3;
        ...
        for (UINT ismp = 0; ismp < pins->samples; ismp++) {
            MODINSTRUMENT *psmp =
                ((ismp < 16) && (smpmap[ismp])) ? &Ins[smpmap[ismp]] : NULL;
            UINT smpnamelen = lpStream[dwMemPos];
            if ((psmp) && (smpnamelen) && (smpnamelen <= 22)) {
                memcpy(m_szNames[smpmap[ismp]], lpStream + dwMemPos + 1,
                       smpnamelen);
            }
    ...
    // Comments
    {
        ...
        if (composernamelen) {
            m_lpszSongComments =
                new char[composernamelen + 1]; // changed from CHAR
            if (m_lpszSongComments) {
                memcpy(m_lpszSongComments, lpStream + dwMemPos + 1,
                       composernamelen);
        ...
        for (UINT i = 0; i < 32; i++) {
            ...
            if ((chnnamlen) && (chnnamlen < MAX_CHANNELNAME)) {
                memcpy(ChnSettings[i].szName, lpStream + dwMemPos + 1,
                       chnnamlen);
            }
    ...
    for (UINT ipat = 0; ipat < psh->patterns; ipat++) {
        ...
        dwMemPos += 4;
        if ((ipat < MAX_PATTERNS) && (packedlen < dwMemLength - dwMemPos) &&
            (numrows >= 8)) {
...
void AMSUnpack(const char *psrc, UINT inputlen, char *pdest, UINT dmax,
               char packcharacter)
{
    UINT tmplen = dmax;
