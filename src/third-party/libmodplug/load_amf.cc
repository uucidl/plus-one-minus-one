...
VOID AMF_Unpack(MODCOMMAND *pPat, const BYTE *pTrack, UINT nRows,
                UINT nChannels)
//-------------------------------------------------------------------------------
{
    UINT lastinstr = 0;
        ...
        // effect
        {
            ...
            case 0x11:
                if (param) {
                    command = CMD_VOLUMESLIDE;
                    if (param & 0x80)
                        param = 0xF0 | ((-(signed char)param) & 0x0F);
                    else
            ...
            case 0x16:
                if (param) {
                    ...
                    if (param & 0x80)
                        param = mask | ((-(signed char)param) & 0x0F);
                    else
                        param |= mask;
                } else
                    command = 0;
...
//-----------------------------------------------------------
{
    ...
    if ((!lpStream) || (dwMemLength < 2048))
        return FALSE;
    if ((!strncmp((LPCTSTR)lpStream, "ASYLUM Music Format V1.0", 25)) &&
        (dwMemLength > 4096)) {
        ...
        for (UINT iSmp = 0; iSmp < numsamples; iSmp++) {
            ...
            psmp->nLoopEnd =
                psmp->nLoopStart + *((LPDWORD)(lpStream + dwMemPos + 33));
        ...
        for (UINT iData = 0; iData < m_nSamples; iData++) {
            ...
            if (psmp->nLength) {
                dwMemPos += ReadSample(
                    psmp, RS_PCM8S, (LPCSTR)(lpStream + dwMemPos), dwMemLength);
            }
        }
        return TRUE;
    }
    ...
    if ((pfh->szAMF[0] != 'A') || (pfh->szAMF[1] != 'M') ||
        (pfh->szAMF[2] != 'F') || (pfh->version < 10) || (pfh->version > 14) ||
        (!pfh->numtracks) || (!pfh->numorders) ||
        (pfh->numorders > MAX_PATTERNS) || (!pfh->numsamples) ||
        (pfh->numsamples > MAX_SAMPLES) || (pfh->numchannels < 4) ||
        (pfh->numchannels > 32))
        return FALSE;
    ...
    if (pfh->version >= 13) {
        if (lpStream[dwMemPos] >= 32)
            m_nDefaultTempo = lpStream[dwMemPos];
        if (lpStream[dwMemPos + 1] <= 32)
            m_nDefaultSpeed = lpStream[dwMemPos + 1];
        dwMemPos += 2;
    }
    // Setup sequence list
    for (UINT iOrd = 0; iOrd < MAX_ORDERS; iOrd++) {
    ...
    if (dwMemPos + m_nSamples * (sizeof(AMFSAMPLE) + 8) > dwMemLength)
        return TRUE;
    // Read Samples
    ...
    for (UINT iTrkMap = 0; iTrkMap < pfh->numtracks; iTrkMap++) {
        if (realtrackcnt < pTrackMap[iTrkMap])
            realtrackcnt = pTrackMap[iTrkMap];
    }
            ...
            if ((nTrack) && (nTrack <= pfh->numtracks)) {
                ...
                if (realtrk) {
                    ...
                    if ((realtrk < realtrackcnt) && (pTrackData[realtrk])) {
                        AMF_Unpack(p + iChn, pTrackData[realtrk],
                                   PatternSize[iPat], m_nChannels);
                    }
                }
            }
        }
    ...
    for (UINT iSeek = 1; iSeek <= maxsampleseekpos; iSeek++) {
        ...
        for (UINT iSmp = 0; iSmp < m_nSamples; iSmp++)
            if (iSeek == sampleseekpos[iSmp]) {
                ...
                dwMemPos +=
                    ReadSample(pins, RS_PCM8U, (LPCSTR)(lpStream + dwMemPos),
