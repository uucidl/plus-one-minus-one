...
//---------------------------------------------------------------------
{
    ...
    case CMD_PORTAMENTODOWN:
        command = 'E';
        if ((param >= 0xE0) && (m_nType & (MOD_TYPE_MOD | MOD_TYPE_XM)))
            param = 0xDF;
        break;
        ...
        if ((bIT) && (m_nType != MOD_TYPE_IT) && (m_nType != MOD_TYPE_XM)) {
            if (param == 0xA4) {
                ...
                command = param = 0;
                   ...
                   ((m_nType == MOD_TYPE_IT) || (m_nType == MOD_TYPE_XM))) {
            param >>= 1;
        }
        break;
    case CMD_PANBRELLO:
...
//---------------------------------------------------------------
{
    ...
    if ((!lpStream) ||
        (dwMemLength <= sizeof(S3MFILEHEADER) + sizeof(S3MSAMPLESTRUCT) + 64))
        return FALSE;
    ...
    if (m_nDefaultSpeed < 1)
        m_nDefaultSpeed = 6;
    if (m_nDefaultSpeed > 0x1F)
        m_nDefaultSpeed = 0x1F;
    // Tempo
    m_nDefaultTempo = psfh.tempo;
    if (m_nDefaultTempo < 40)
        m_nDefaultTempo = 40;
    if (m_nDefaultTempo > 240)
        m_nDefaultTempo = 240;
    // Global Volume
    ...
    if ((iord & 1) && (lpStream[dwMemPos] == 0xFF))
        dwMemPos++;
    ...
    for (UINT iSmp = 1; iSmp <= insnum; iSmp++) {
        ...
        if ((s[0] == 1) && (s[0x4E] == 'R') && (s[0x4F] == 'S')) {
            ...
            if (j >= Ins[iSmp].nLength)
                j = Ins[iSmp].nLength - 1;
    ...
    for (UINT iRaw = 1; iRaw <= insnum; iRaw++)
        if ((Ins[iRaw].nLength) && (insfile[iRaw])) {
            ...
            if (insflags[iRaw - 1] & 4)
                flags += 5;
            if (insflags[iRaw - 1] & 2)
                flags |= RSF_STEREO;
            if (inspack[iRaw - 1] == 4)
                flags = RS_ADPCM4;
            dwMemPos = insfile[iRaw];
            dwMemPos +=
                ReadSample(&Ins[iRaw], flags, (LPSTR)(lpStream + dwMemPos),
...
static BYTE S3MFiller[16] = {0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80,
...
//----------------------------------------------------------
{
    ...
    for (i = 0; i < MAX_ORDERS; i++)
        if ((Order[i] < MAX_PATTERNS) && (Order[i] >= nbp))
            nbp = Order[i] + 1;
    ...
    header[0x33] =
        ((m_nSongPreAmp < 0x20) ? 0x20 : m_nSongPreAmp) | 0x80; // Stereo
    header[0x35] = 0xFC;
    for (i = 0; i < 32; i++) {
        if (i < m_nChannels) {
            UINT tmp = (i & 0x0F) >> 1;
    ...
    for (i = 0; i < nbi; i++)
        insptr[i] = (WORD)((ofs + i * 0x50) / 16);
    for (i = 0; i < nbp; i++)
        patptr[i] = (WORD)((ofs + nbi * 0x50) / 16);
    ...
    for (i = 1; i <= nbi; i++) {
        ...
        if (pins->pSample) {
            ...
            if (pins->nC4Speed)
                ...
                insex[i - 1].finetune =
                    TransposeToFrequency(pins->RelativeTone, pins->nFineTune);
            UINT flags = RS_PCM8U;
#ifndef NO_PACKING
            if (nPacking) {
