...
typedef struct PTMFILEHEADER {
    ...
    BYTE chnpan[32];  // channel panning settings, 0..15, 0 = left, 7 = middle,
                      // 15 = right
...
//---------------------------------------------------------------
{
    ...
    if ((pfh.ptmf_id != 0x464d5450) || (!pfh.nchannels) ||
        (pfh.nchannels > 32) || (pfh.norders > 256) || (!pfh.norders) ||
        (!pfh.nsamples) || (pfh.nsamples > 255) || (!pfh.npatterns) ||
        (pfh.npatterns > 128) ||
    ...
    for (UINT ismp = 0; ismp < m_nSamples;
         ismp++, dwMemPos += SIZEOF_PTMSAMPLE) {
        ...
        if ((psmp->sampletype & 3) == 1) {
            ...
            if ((pins->nLength) && (samplepos) && (samplepos < dwMemLength)) {
                ReadSample(pins, smpflg, (LPSTR)(lpStream + samplepos),
                           dwMemLength - samplepos);
            }
        }
    }
    // Reading Patterns
