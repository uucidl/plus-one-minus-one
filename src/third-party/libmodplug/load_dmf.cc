...
//---------------------------------------------------------------
{
    ...
    Log("DMF version %d: \"%s\": %d bytes (0x%04X)\n", pfh->version,
        m_szNames[0], dwMemLength, dwMemLength);
#endif
    while (dwMemPos + 7 < dwMemLength) {
        ...
        case 0x47534d43:
            ...
            if ((psi->infosize >= 8) && (!m_lpszSongComments)) {
                m_lpszSongComments =
                    new char[psi->infosize]; // changed from CHAR
        ...
        case 0x55514553:
                ...
                goto dmfexit;
            {
                ...
                if (sequ->loopstart < nseq)
                    m_nRestartPos = sequ->loopstart;
                for (UINT i = 0; i < nseq; i++)
                    Order[i] = (BYTE)sequ->sequ[i];
            }
        ...
        case 0x54544150:
            if (!m_nChannels) {
                ...
                if (m_nChannels < patt->firstpatinfo)
                    m_nChannels = patt->firstpatinfo;
                if (m_nChannels > 32)
                    m_nChannels = 32;
                if (m_nChannels < 4)
                    m_nChannels = 4;
                for (UINT npat = 0; npat < numpat; npat++) {
                    ...
                    MODCOMMAND *m =
                        AllocatePattern(PatternSize[npat], m_nChannels);
                    if (!m)
                        goto dmfexit;
                    Patterns[npat] = m;
                    DWORD d = dwPos;
                    ...
                    for (UINT row = 0; row < ticks; row++) {
                        ...
                        if (tempochange) {
                            ...
                            UINT rpm = ((ttype) && (pbeat)) ? tempo * pbeat
                                                            : (tempo + 1) * 15;
...
#ifdef DMFLOG
                            Log("Tempo change: ttype=%d pbeat=%d tempo=%3d -> "
                                "speed=%d tempo=%d\n",
        ...
        case 0x49504d53: {
            ...
            if (pds->size <= dwMemLength - dwMemPos) {
                ...
                for (UINT iSmp = 1; iSmp <= m_nSamples; iSmp++) {
                    ...
                    psmp->nGlobalVol = 64;
                    psmp->nVolume =
                        (psh->volume) ? ((WORD)psh->volume) + 1 : (WORD)256;
        ...
        case 0x44504d53: {
            ...
            for (UINT iSmp = 1; iSmp <= m_nSamples; iSmp++) {
                ...
                Log("len=%d flags=0x%X [%08X]\n", Ins[iSmp].nLength,
                    smplflags[ismpd], *((LPDWORD)(lpStream + dwPos + 4)));
#endif
                dwPos += 4;
                ...
                if ((pksize) && (iSmp <= m_nSamples)) {
                    UINT flags =
                        (Ins[iSmp].uFlags & CHN_16BIT) ? RS_PCM16S : RS_PCM8S;
                    if (smplflags[ismpd] & 4)
                        flags =
                            (Ins[iSmp].uFlags & CHN_16BIT) ? RS_DMF16 : RS_DMF8;
                    ReadSample(&Ins[iSmp], flags, (LPSTR)(lpStream + dwPos),
                               pksize);
                }
                dwPos += pksize;
            }
