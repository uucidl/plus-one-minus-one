...
//---------------------------------------------------------------
{
    ...
    if ((pfh->okta != 0x41544B4F) || (pfh->song != 0x474E4F53) ||
        ...
        (pfh->fixed8 != 0x08000000) || (pfh->samp != 0x504D4153))
        return FALSE;
    m_nType = MOD_TYPE_OKT;
    m_nChannels = 4 + pfh->chnsetup[1] + pfh->chnsetup[3] + pfh->chnsetup[5] +
                  pfh->chnsetup[7];
    ...
    if (*((DWORD *)(lpStream + dwMemPos)) == 0x54544150) {
        ...
        for (UINT i = 0; i < orderlen; i++)
            Order[i] = lpStream[dwMemPos + 10 + i];
    ...
    while ((dwMemPos + 10 < dwMemLength) &&
           (*((DWORD *)(lpStream + dwMemPos)) == 0x444F4250)) {
    ...
    UINT nsmp = 1;
    while ((dwMemPos + 10 < dwMemLength) &&
           (*((DWORD *)(lpStream + dwMemPos)) == 0x444F4253)) {
        if (nsmp < MAX_SAMPLES)
            ReadSample(&Ins[nsmp], RS_PCM8S, (LPSTR)(lpStream + dwMemPos + 8),
                       dwMemLength - dwMemPos - 8);
        dwMemPos += bswapBE32(*((DWORD *)(lpStream + dwMemPos + 4))) + 8;
        nsmp++;
    }
    return TRUE;
