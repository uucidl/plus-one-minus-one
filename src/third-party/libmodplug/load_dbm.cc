...
//---------------------------------------------------------------
{
    ...
    if ((!lpStream) || (dwMemLength <= sizeof(DBMFILEHEADER)) ||
        (!pfh->channels) || (pfh->dbm_id != DBM_FILE_MAGIC) || (!pfh->songs) ||
        (pfh->song_id != DBM_ID_SONG) || (pfh->name_id != DBM_ID_NAME) ||
        (pfh->name_len != DBM_NAMELEN) || (pfh->info_id != DBM_ID_INFO) ||
        (pfh->info_len != DBM_INFOLEN))
        return FALSE;
    ...
    if (m_nChannels > 64)
        m_nChannels = 64;
    memcpy(m_szNames[0], (pfh->songname[0]) ? pfh->songname : pfh->songname2,
           32);
    m_szNames[0][31] = 0;
    for (UINT iOrd = 0; iOrd < nOrders; iOrd++) {
        Order[iOrd] = lpStream[dwMemPos + iOrd * 2 + 1];
        if (iOrd >= MAX_ORDERS - 2)
            break;
    }
    dwMemPos += 2 * nOrders;
    while (dwMemPos + 10 < dwMemLength) {
        ...
        dwMemPos += 8;
        chunk_pos = dwMemPos;
        if ((dwMemPos + chunk_size > dwMemLength) || (chunk_size > dwMemLength))
            break;
        dwMemPos += chunk_size;
        // Instruments
        if (chunk_id == DBM_ID_INST) {
            ...
            for (UINT iIns = 0; iIns < nInstruments; iIns++) {
                ...
                if (chunk_pos + sizeof(DBMINSTRUMENT) > dwMemPos)
                    break;
                if ((penv = new INSTRUMENTHEADER) == NULL)
                    break;
            ...
            for (UINT iEnv = 0; iEnv < nEnvelopes; iEnv++) {
                ...
                if ((nins) && (nins < MAX_INSTRUMENTS) && (Headers[nins]) &&
                    (peh->numpoints)) {
                    ...
                    for (UINT i = 0; i < penv->nVolEnv; i++) {
                        ...
                        penv->VolEnv[i] =
                            (BYTE)bswapBE16(peh->volenv[i * 2 + 1]);
                    }
                }
            ...
            for (UINT iSmp = 1; iSmp <= nSamples; iSmp++) {
                ...
                if ((chunk_pos + samplesize > dwMemPos) ||
                    ...
                    break;
                if (sampleflags & 3) {
                    ReadSample(pins, (pins->uFlags & CHN_16BIT) ? RS_PCM16M
                                                                : RS_PCM8S,
