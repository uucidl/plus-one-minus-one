...
static int vlogic_new(effect_t *self)
{
...
/* generate palette */
#define GREY(a)                                                                \
    ((((a) << 16) & 0xff0000) | (((a) << 8) & 0x00ff00) | (((a)) & 0xff))
...
static void vlogic_set_frame_size(video_effect_t *self, int width, int height,
                                  int pitch)
{
    vlogic_t *v = (vlogic_t *)self;
    self->width = width;
...
static void vlogic_set_multiplier(vlogic_t *self, unsigned int i,
                                  double multiplier)
{
    switch (i) {
    case 0: {
...
static void vlogic_generate_palette(vlogic_t *self, palette_gen_f gen)
{
    if (gen) {
        unsigned int i;
        for (i = 0; i < 256; i++) {
...
static inline uint32_t palette_lookup(vlogic_t *self, unsigned char i)
{
    return self->palette[i];
}

#ifdef USE_CG
static void vlogic_computes_cg(effect_t *self, void *content, double ms)
{
        ...
        if (n >= nz) {
            ...
            while (count) {
                if (count >= ny) {
                    ...
                    while (count2--) {
                        pixels[x++] =
                            palette_lookup(vlogic, vlogic->block[i++]);
                        if (x == width) {
                            x = 0;
                            pixels += pitch;
                        }
            ...
            while (count > 0) {
                if (count >= ny) {
                    ...
                    while (count2-- > 0) {
                        ...
                        pixels[x++] =
                            palette_lookup(vlogic, vlogic->state.accum);
                        if (x == width) {
                            x = 0;
                            pixels += pitch;
                        }
