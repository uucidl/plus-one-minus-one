...
static int outline_new(outline_t *self)
{
...
static int outline_destroy(outline_t *self)
{
...
static void outline_add_point(outline_t *self)
{
    self->ftoutline.n_points++;
    self->ftoutline.points = realloc(
        self->ftoutline.points, self->ftoutline.n_points * sizeof(FT_Vector));
    self->ftoutline.tags =
        realloc(self->ftoutline.tags, self->ftoutline.n_points * sizeof(char));
}

static void outline_add_contour(outline_t *self)
{
    self->ftoutline.n_contours++;
    self->ftoutline.contours = realloc(
        self->ftoutline.contours, self->ftoutline.n_contours * sizeof(short));
}

static outline_t *outline_copy(outline_t *self, outline_t *src)
{
...
static inline FT_Pos float2Pos(float f)
{
    FT_Pos pos = f * 64.f;
    return pos;
}

static outline_t *outline_move_to(outline_t *self, vector2d_t *to)
{
...
static outline_t *outline_line_to(outline_t *self, vector2d_t *to)
{
    /* adds a new point */
    outline_add_point(self);
    ...
    self->ftoutline.contours[self->ftoutline.n_contours - 1] =
        self->ftoutline.n_points - 1;

    return self;
}

static outline_t *outline_conic_to(outline_t *self, vector2d_t *control,
                                   vector2d_t *to)
{
    outline_add_point(self);
    self->ftoutline.points[self->ftoutline.n_points - 1].x =
        float2Pos(control->x);
    self->ftoutline.points[self->ftoutline.n_points - 1].y =
        float2Pos(control->y);
...
static outline_t *outline_cubic_to(outline_t *self, vector2d_t *control1,
                                   vector2d_t *control2, vector2d_t *to)
{
    outline_add_point(self);
    self->ftoutline.points[self->ftoutline.n_points - 1].x =
        float2Pos(control1->x);
    self->ftoutline.points[self->ftoutline.n_points - 1].y =
        float2Pos(control1->y);
    self->ftoutline.tags[self->ftoutline.n_points - 1] = 2;

    outline_add_point(self);
    self->ftoutline.points[self->ftoutline.n_points - 1].x =
        float2Pos(control2->x);
    self->ftoutline.points[self->ftoutline.n_points - 1].y =
        float2Pos(control2->y);
    self->ftoutline.tags[self->ftoutline.n_points - 1] = 2;
...
static outline_t *outline_scale(outline_t *self, vector2d_t *scaling)
{
...
static outline_t *outline_rotate(outline_t *self, double angle)
{
...
static outline_t *outline_translate(outline_t *self, vector2d_t *dis)
{
    FT_Outline_Translate(&self->ftoutline, float2Pos(dis->x),
                         float2Pos(dis->y));
