 ...
 *
 * You should have received this file ('src/freetype/ft_renderer.c') with a license
...
static
int ft_destroy(ft_renderer_t* self)
{
...
static
unsigned long io_read(FT_Stream ft_stream, unsigned long offset, unsigned char* buffer,
		      unsigned long   count )
{
...
static
void io_close(FT_Stream ft_stream)
{
    stream_t* stream = ft_stream->descriptor.pointer;
...
static
FT_Stream create_ft_stream(stream_t* stream)
{
...
#ifdef FT_LOAD_FONT
static
atom_t ft_load_font(ft_renderer_t* self, const char* filename)
{
    int ret = 0;
    int error;
...
static
atom_t ft_load_font_from_stream(ft_renderer_t* self, stream_t* stream)
{
    atom_t ret = 0;
    int error;
	...
	return 0;
	  ...
	  error =
	      FT_Select_Charmap(self->faces[self->faces_next_index], ft_encoding_unicode);
	  if(error)
...
static
void ft_use_font(ft_renderer_t* self, atom_t id)
{
    if(id) {
...
static
void ft_draw_span(int y, int count, FT_Span const* spans, void* user)
{
  void** data;
  ft_renderer_t* self;
  image_t* image;
  ...
  { int i; for(i=0; i<count; i++) {
      ...
      if (self->must_overwrite_p) {
	      const unsigned int c = (self->color & 0xffffff) | (span->coverage << 24);
	      for (j = 0; j < len; j++) {
		      start [j] = c;
	      }
      } else {
	      const unsigned int alpha = (((self->color & 0xff000000) >> 16) * span->coverage) >> 16;
...
static
void ft_compute_string_extent (ft_renderer_t* self, image_t* destination, string_t* string) {
    FT_GlyphSlot     slot;
    FT_Face face;
...
static
void ft_draw_string_direct(ft_renderer_t* self, image_t* image, string_t* string)
{
    void* user_data[2];
    user_data[0] = self;
    user_data[1] = image;
...
static
void ft_draw_outline(ft_renderer_t* self,
		     image_t* image, outline_t* outline)
{
    FT_Raster_Params params;
    int error;
    void* user_data[2];
	...
	DEBUG5("clip_box: x: %d - %d, y: %d - %d\n",
	       params.clip_box.xMin >> 6, params.clip_box.xMax >> 6,
	       params.clip_box.yMin >> 6, params.clip_box.yMax >> 6);
    }
#endif
...
static
void oi_draw_span(int y, int count, FT_Span const* spans, void* user)
{
  ...
  { int i; for(i=0; i<count; i++) {
	  ...
	  if(i_x >= src_image->width)
	      ...
	      i_len += i_x;
	      /* correct starting position to account for clipping of the source image */
	      start -= i_x;
	      i_x = 0;
	  }
      ...
      {
	  unsigned int alpha = (((self->color & 0xff000000) >> 16) * span->coverage) >> 16;
	  int j;
...
static
void ft_draw_outlined_image(ft_renderer_t* self,
			    image_t* image, outline_t* outline,
...
static
void ft_draw_cstring(ft_renderer_t* self, image_t* dest, const char* str)
{
    string_t string;
...
static
void draw_blend_bitmap(image_t* dest, FT_Bitmap* src, int x, int y)
{
...
static
void ft_draw_string(ft_renderer_t* self, image_t* image, string_t* string)
{
    FT_GlyphSlot  slot;
    int error;
    FT_Face face;
	...
	draw_blend_bitmap( image,
			   &slot->bitmap,
			   self->pen_x + slot->bitmap_left,
