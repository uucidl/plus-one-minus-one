...
static
void choose_center(icefx_t* icefx)
{
...
static
void icefx_throw(icefx_t* self, double now)
{
    int i;
	...
	self->ice[i].create(&self->ice[i], gamma,
			    var+400.0*unirand(), r*self->rmax + 75.0*unirand());
...
static
void icefx_computes(effect_t* e, void* content, double ms)
{
  icefx_t* icefx = (icefx_t*) e;
  ...
  for(i=0; i<icefx->count; i++) {
	  ...
	  f = 0.04*(60000.0 - (ms-80000.0))/60000.0*(ms-icefx->onset_ms) / icefx->ice[i].end_ms;
	  icefx->renderer.pen_x += f*(icefx->ice[i].x - icefx->center.x);
	  icefx->renderer.pen_y += f*(icefx->ice[i].y - icefx->center.y);
      }
      icefx->renderer.color = pixel_lerp(icefx->alpha, icefx->ice[i].color, 0x00000);
