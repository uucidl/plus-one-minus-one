...
static
void add_url(url_fetcher_t* self, url_t* url)
{
    ...
    if(url->path && strlen(url->path) && url->path[strlen(url->path)-1] == '/') {
...
static
int url_not_seen(url_fetcher_t* self, url_t* url)
{
    iterator_t i;
    url_t* url_slot;
    char* query = NULL;
...
static
void add_collected_url(url_fetcher_t* self, char* url)
{
    char** string_slot;
...
int images_google_com_callback(url_fetcher_t* self, http_stream_t* url_found)
{
	...
	if(n > 3 && !strcmp(url_found->url.path+n-3, "jpg")) {
	    ...
	    */
		  ...
		  (!url_found->url.query || !strstr(url_found->url.query, "imgsafe=on")) &&
		  url_not_seen(self, &url_found->url)) {
	    if(url_found->url.query && strstr(url_found->url.query, "imgurl=")) {
		char* imgurl;
		char* imgurl_begin =
...
int http_directory_callback(url_fetcher_t* self, http_stream_t* url_found)
{
	...
	url_found->stat(url_found);
	if(url_found->status == 200) {
	    if(url_found->content_type && (strstr(url_found->content_type, "image/jpeg") ||
					   strstr(url_found->content_type, "image/png"))) {
		add_collected_url(self, url_found->url.create_string(&url_found->url));
	    } else if(!strstr(url_found->url.path, "..") &&
		      url_not_seen(self, &url_found->url) &&
		      url_found->url.is_subdir_of(&url_found->url, &self->from_url->url)) {
...
static
void parse_links(url_fetcher_t* state, http_stream_t* stream, int depth)
{
    if(depth < state->guard &&
       stream && stream->status == 200 &&
       stream->content_type &&
       strstr(stream->content_type, "text/html")) {
	stream_t* s;
	enum { TAG_IN, TAG_NOTA, IN_A, TAG_A,
	       H, HR, HRE, HREF,
	       EQUALS, LINK, QUOTE,
	       TAG_OUT } status = TAG_OUT, new_status = TAG_OUT;
	int index = 0;
	char link_url[256];
	char character;
	int just_in_p = 1;
	...
	TRACE4("parsing... '%s%s?%s'\n",
	       stream->url.server, stream->url.path, stream->url.query);
	...
	while(stream_get_callbacks(s)->read (&character, 1, 1, s)) {
		...
		if(status == LINK) {
		    if(state->callback) {
			http_stream_t* stream2 = http_stream_instantiate_toplevel (NULL);

			link_url[index] = '\0';

			TRACE2 ("url found: %s", link_url);

			stream2->url.new2(&stream2->url, &stream->url, link_url);
			...
			if(state->callback (state, stream2)) {
				...
				stream_get_callbacks(&effective_stream->super.super)
