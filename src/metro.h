...
typedef struct metronome_t {
    ...
    int (*new)(struct metronome_t *self, double startms, double periodms,
               double resolutionms);
    void (*reinit)(struct metronome_t *self, double startms, double periodms,
                   double resolutionms);
    double period_ms;
...
typedef struct swinger_t {
    ...
    int (*new)(struct swinger_t *self, unsigned int beats,
               beatsource_t *beatsource);
    /* beat's origin is 0 */
    void (*set_beat)(struct swinger_t *self, unsigned int beat,
                     unsigned int times);
