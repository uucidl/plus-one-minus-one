...
static
void stream_read_data(png_structp png_ptr,
		      png_bytep data,
		      png_uint_32 length)
{
...
image_t* image_load_png(image_t* x, stream_t* stream)
{
    ...
    if(stream_get_callbacks(stream)
       ->read(header, 1, header_n, stream) < (unsigned int) header_n) {
	if(!x) free(im);
	return NULL;
    }
	...
	(BYTE_ORDER == BIG_ENDIAN && defined (PIXEL_BGRA8888))
		...
		for(i=0; i<im->height; i++)
			row_pointers[i] = calloc (im->width * sizeof(uint32_t), sizeof(unsigned char));
