 ...
 *
 * You should have received this file ('src/messaging/receiver.c') with a license
...
static
message_fifo_t* get_fifo(receiver_t* self, atom_t a)
{
    map_value_t pfifo = map_get (&self->fifos, (unsigned long) a);
    message_fifo_t* fifo = map_value_is_there (pfifo) ? map_value_obtain (pfifo) : NULL;
    
    return fifo;
}

static
message_fifo_t* add_fifo(receiver_t* self, atom_t a)
{
...
static
message_node_t* node_iterator_next(node_iterator_t* self)
{
...
static
message_node_t* node_iterator_prev(node_iterator_t* self)
{
    message_node_t* node = self->node;
    if(self->node) {
...
static
node_iterator_t* get_node_iterator(message_fifo_t* fifo, node_iterator_t* x)
{
...
static
node_iterator_t* get_node_iterator_end(message_fifo_t* fifo, node_iterator_t* x)
{
    node_iterator_t* it = x ? x : malloc(sizeof(node_iterator_t));
    memset(it, 0, sizeof(node_iterator_t));
...
/*--- receiver ---*/
static
void receiver_eval(receiver_t* self,
		   bytecode_stream_t* message, context_t* context)
{
    self->dispatcher.dispatch(&self->dispatcher, 
			      context,
			      message);
...
static
void receiver_eval_pending(receiver_t* self, 
			   atom_t selector, context_t* context)
{
...
static
void receiver_eval_all_pending(receiver_t* self, context_t* context)
{
    map_iterator_t it; 
    map_value_t pfifo;
...
static
message_node_t* receiver_peek_pending(receiver_t* self, 
				      atom_t selector, context_t* context)
{
    message_fifo_t* fifo = get_fifo(self, selector);
    node_iterator_t it;
...
static
message_node_t* receiver_peek_any_pending(receiver_t* self, context_t* context)
{
    map_iterator_t   it; 
    message_node_t*  head = NULL;
    map_value_t pfifo;
...
static
void receiver_receive(receiver_t* self, 
		      bytecode_stream_t* message, context_t* context)
{
    atom_t selector;
    message_fifo_t* fifo;
    ...
    */
	...
	TRACE3("adding at: %f in fifo: %s", context->ms, atom_get_cstring_value(fifo->atom));
...
static 
void receiver_receive_for_self(receiver_t* self, 
			       bytecode_stream_t* message, context_t* context)
{
...
static
void receiver_open(receiver_t* self, atom_t selector)
{
    message_fifo_t* fifo = get_fifo(self, selector);
    if(!fifo) {
	fifo = add_fifo(self, selector);
...
static
void receiver_close(receiver_t* self, atom_t selector)
{
    message_fifo_t* fifo = get_fifo(self, selector);
    if(fifo) {
	fifo->closed_p = 1;
...
static
void receiver_set_definition(struct receiver_t* self, 
			     atom_t a, bytecode_stream_t* args, recp_f f)
{
    ((definitions_t*) &self->dispatcher.definitions)->set_callback
	((definitions_t*) &self->dispatcher.definitions, a, args, f);
