...
static
void stutter_set_block_size(stutter_t* self, unsigned int samples)
{
    if(samples > 65536) {
	WARNING2("tried to set block size to an incorrect value. ('%u')", samples);
	samples = 65535;
    } else if(samples == 0) {
	samples = 2;
    }
...
static
void stutter_set_window(stutter_t* self, stutter_window_t window)
{
  ...
  if(self->window_p) {
      ...
      self->window->set_period(self->window,  1.0 * self->block_size / self->super.sample_rate);
  }
}

static
void stutter_set_speed(stutter_t* self, double speed)
{
    self->phase_increment = speed * 65536;
}

static void stutter_mono_computes_area(audio_effect_t* e, audio_area_t* area, double ms) __attribute__((unused));

static
void stutter_mono_computes_area(audio_effect_t* e, audio_area_t* area, double ms)
{
  ...
  /* algo:
     for each sample, grab value if not repeat, put it at current position in the back block.
     put value of block to current sample. when back block is full, exchange back block and block
     pointers
  */
  for(i=0; i<area->frame_number; i++) {
	...
	samples[i*self->super.frame_size] = samples[i*self->super.frame_size + 1] = 0.0f;
    }
...
static
void stutter_stereo_computes_area(audio_effect_t* e, audio_area_t* area, double ms)
{
  stutter_t* self = (stutter_t*) e;
  sample_t* samples = area->samples + area->head*self->super.frame_size;
  ...
  */
  for(i=0; i<area->frame_number; i++) {
    ...
    if(!self->repeat_l_p) {
      ...
      self->bblock[2*self->bblock_phase] = self->block[2*self->bblock_phase];
    }
    ...
    if(!self->repeat_r_p) {
      ...
      self->bblock[2*self->bblock_phase+1] = self->block[2*self->bblock_phase+1];
    }
...
static
void stutter_set_repeat_probability(stutter_t* self, double proba)
{
  if(proba > 1.0)
    proba = 1.0;
  else if(proba < 0.0)
...
static
void stutter_set_dropout_probability(stutter_t* self, double proba)
{
  if(proba > 1.0)
    proba = 1.0;
  else if(proba < 0.0)
...
static
double stutter_get_latency_ms(effect_t* zelf)
{
	...
	return 1000. *
	    (self->block_size - (self->block_phase >> 16)) / 44100.;
    }
}
