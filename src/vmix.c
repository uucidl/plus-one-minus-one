...
static
void vmix_set_frame_size(video_effect_t* v, int width, int height, int pitch)
{
	...
	if(vmix->left_effect)
	    vmix->left_effect->set_frame_size
		(vmix->left_effect, width, height, width);

	if(vmix->right)
	    free(vmix->right);
	vmix->right = calloc(width*height, sizeof(int32_t));
	if(vmix->right_effect)
	    vmix->right_effect->set_frame_size
		(vmix->right_effect, width, height, width);
    }
}

static
void vmix_set_mix_type_cb(vmix_t* self, atom_t type)
{
    enum vmix_type t;
...
static
void vmix_set_mix_type(vmix_t* self, enum vmix_type type)
{
    self->type = type;
}

static
void vmix_set_mix(vmix_t* self, float factor)
{
    if(factor < 0.0f) 
	factor = 0.0f;
    if(factor > 1.0f)
...
static
void vmix_set_left_effect(vmix_t* self, video_effect_t* e)
{
    self->left_effect = e;
    if(e)
	e->set_frame_size(e, self->super.width, self->super.height, self->super.width);
}

static
void vmix_set_right_effect(vmix_t* self, video_effect_t* e)
{
    self->right_effect = e;
    if(e)
	e->set_frame_size(e, self->super.width, self->super.height, self->super.width);
...
static
void vmix_set_left_offset_ms(vmix_t* self, double ms)
{
    self->left_ms = ms;
}

static
void vmix_set_right_offset_ms(vmix_t* self, double ms)
{
    self->right_ms = ms;
}

static inline
void blit_amp_add(vmix_t* self, 
		  int32_t* __restrict__ pixelsd, int32_t* __restrict__ pixelss, 
		  double amp)
{
	...
	*pixelsd += ((int)(amp*r) << 16) | ((int)(amp*g)<<8) | (int)(amp*b);
	pixelss++;
	pixelsd++;
    }
}
...
static
void vmix_computes(effect_t* self, void* content, double ms)
{
    ...
    if(vmix->mix >= 1.0f) {
	if(vmix->left_effect)
	    ((effect_t*)vmix->left_effect)->computes
		((effect_t*)vmix->left_effect, pixelsl, ms-vmix->left_ms);
	{
	    int h = height;
	    for(h=0; h<height; h++)
		memcpy(pixelsd+h*pitch, pixelsl+h*width, width*sizeof(int32_t));
	}
    } else if(vmix->mix <= 0.0f) {
	if(vmix->right_effect)
	    ((effect_t*)vmix->right_effect)->computes
		((effect_t*)vmix->right_effect, pixelsr, ms-vmix->right_ms);
	{
	    int h = height;
	    for(h=0; h<height; h++) 
		memcpy(pixelsd+h*pitch, pixelsr+h*width, width*sizeof(int32_t));
	}
    } else { 
	/* then do it */
	if(vmix->left_effect)
	...
	if(vmix->type == CROSSFADE) {
		...
		//hw = (0.5+0.5*cos(M_PI_2*h/height+M_PI_2*phase))*hwater; /* sinus? */
...
static
int vmix_destroy(effect_t* self)
{
     vmix_t* vmix = (vmix_t*) self;
     if(vmix->left) {
       free(vmix->left);
...
vmix_t* vmix_instantiate(vmix_t* x)
{
    ...
    {
	receiver_t* r   = router_get_receiver(effect_get_router(&vm->super.super));
	dictionary_t* d = dictionary_get_instance();
	
	r->set_definition(r, d->new_atom(d, "set-mix"), compile_cstring("integer", NULL),   (recp_f) vm->set_mix);
	r->set_definition(r, d->new_atom(d, "set-mix-type"), compile_cstring("atom", NULL), (recp_f) vmix_set_mix_type_cb);
    }
