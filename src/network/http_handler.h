 ...
 *
 * You should have received this file ('src/network/http_handler.h') with a license
...
typedef struct http_handler_t
{
    ...
    /* parsers: */
    /* status line parser */
    enum { ST_VERSION, ST_SP1, 
	   ST_STATUS, ST_SP2, 
	   ST_REASON, 
	   ST_CR, ST_CRLF, ST_WEIRD } st_status, st_next_status;
    char st_code[4];
    int  st_index;
    /* returns number of char processed */
    int (*st_parse_block)(struct http_handler_t* self, 
			  char* buffer, int n);
    
    /* header parser */
    enum { HDR_CR, HDR_CRLF, HDR_CRLFCR, HDR_CRLFCRLF, 
	   HDR_FIELD, HDR_COLON, 
	   HDR_VALUE, HDR_WEIRD } hdr_status, hdr_next_status;
    char hdr_field[256];
    char hdr_value[256];
    int hdr_index;
    int (*hdr_parse_block)(struct http_handler_t* self,
			   char* buffer, int n);
    ...
    int   bdy_size;
    int   bdy_index;
    int (*bdy_decode_block)(struct http_handler_t* self,
			    char* buffer, int n);

    /* chunk decoder */
    enum { BDY_CHUNK_SIZE, BDY_CHUNK_EXT,
	   BDY_CHUNK_CR, BDY_CHUNK_CRLF, 
	   BDY_CHUNK_DATA, BDY_CHUNK_DATA_END, 
	   BDY_CHUNK_DATA_CR,
	   BDY_CHUNK_DATA_CRLF, 
	   BDY_CHUNK_TRAILER, BDY_CHUNK_WEIRD } chnk_status, chnk_next_status;
    ...
    int (*bdy_decode_chunked_block)(struct http_handler_t* self,
				    char* buffer, int n);
    
    /* trailer decoder */
    enum { TRLR, TRLR_TRLR, TRLR_CR, TRLR_CRLF, 
	   TRLR_CRLFCR, TRLR_CRLFCRLF } chnk_trlr_status;
} http_handler_t;
