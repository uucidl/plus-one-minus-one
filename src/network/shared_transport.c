 ...
 *
 * You should have received this file ('src/network/shared_transport.c') with a license
...
static
cached_channel_t* channel_cache_get_channel()
{
...
static
cached_channel_t* channel_cache_get_channel_by_url(url_t* url)
{
    cached_channel_t* ret = NULL;
    iterator_t it;
...
static
void channel_cache_release_channel(cached_channel_t* cs)
{
    int err;
...
static
void get_entry_iterator(shared_transport_t* self, entry_iterator_t* it)
{
    it->parent  = self;
...
static
channel_entry_t* entry_iterator_next(entry_iterator_t* it)
{
...
static
void add_entry(shared_transport_t* self, channel_entry_t* e)
{
    if(!self->end) {
	e->next = e;
	self->end = e;
...
static
void remove_entry(shared_transport_t* self, channel_entry_t* e)
{
...
static
void ts_register_channel(selector_t* zelf,
			 channel_t* channel,
...
static
void internal_unregister_channel(selector_t* zelf, channel_t* channel)
{
    tselector_t* self = (tselector_t*) zelf;
...
static
void ts_unregister_channel(selector_t* zelf, channel_t* channel)
{
    tselector_t* self = (tselector_t*) zelf;
...
static
void ts_nop(selector_t* zelf)
{}

typedef void* (*pthread_run_f)(void*);

static
void* ts_run(selector_t* zelf)
{
    tselector_t* self = (tselector_t*) zelf;
	...
	if(n == 0) {
	    ...
	    pthread_cond_wait (&self->first_fd_registered,
			       &self->mutex);
	    TRACE1 ("done");
	    n = self->n;
	}
	...
	if(n < 0) {
		...
		if(c) {
			...
			// pass to callback
			if(cbs && cbs->write) {
			    int nbytes = cbs->write(cbs, c, self->buffer, BUFSIZE);
			    // write to socket
			    if(nbytes) {
				int n = 0;
				n = stream_get_callbacks(&stream->super)
				    ->write(self->buffer, 1, nbytes, &stream->super);
...
static
tselector_t* tselector_instantiate(tselector_t* x)
{
	...
	if(kn_thread_create_minimum_priority_with_attributes (&id, &attr, ts_pthread_run, ts)) {
...
static
channel_t* transport_connect_to(transport_t* zelf,
				url_t* url,
				transport_callbacks_t* callbacks)
{
...
static
void transport_release(transport_t* zelf, channel_t* channel)
{
    shared_transport_t* self = (shared_transport_t*) zelf;
	...
	if(ce->pair && &ce->pair->channel == channel) {
	    ...
	    if(ce->selector)
		ce->selector->unregister_channel(ce->selector, &ce->pair->channel);
	    break;
	}
    }
...
static
void transport_run(transport_t* zelf, selector_t* selector)
{
    shared_transport_t* self = (shared_transport_t*) zelf;
    entry_iterator_t it;
    channel_entry_t* c;
	...
	if(!c->selector && c->pair && c->callbacks) {
	    selector->register_channel(selector, &c->pair->channel, c->callbacks);
	    c->selector = selector;
	}
    }
    pthread_mutex_unlock(&self->entries_mutex);
