 ...
 *
 * You should have received this file ('src/network/http_handler.c') with a license
...
static
void set_state(http_handler_t* self, http_state_t nu)
{
...
static
http_state_t get_state(http_handler_t* self)
{
    http_state_t state;
    if(pthread_mutex_lock(&self->state_mutex))
	ERROR1("state_mutex lock");
...
static
void hh_new(http_handler_t* self, http_response_t* response)
{
...
static
void prepare_request(http_handler_t* self, const char* format, url_t* url)
{
	...
	self->request = malloc(strlen(format) + strlen(temp) + strlen(self->url.server));
	sprintf(self->request, format, temp, self->url.server);
    } else {
	self->request = malloc(strlen(format) + 
			       strlen(self->url.path) + 
...
static
void hh_prepare_head_request(http_handler_t* self, url_t* url)
{
    const char* format = 
	"HEAD %s HTTP/1.1\r\nHost: %s\r\nUser-Agent: +1-1 (http://neq.tpolm.com/)\r\n"
	//"Connection: close\r\n"
	"\r\n";
...
static
void hh_prepare_get_request(http_handler_t* self, url_t* url)
{
    const char* format = 
	"GET %s HTTP/1.1\r\nHost: %s\r\nUser-Agent: +1-1 (http://neq.tpolm.com/)\r\n"
	//"Connection: close\r\n"
...
static
int hh_on_write(transport_callbacks_t* zelf, channel_t* c, char* buffer, int n)
{
...
static
int hh_st_parse_block(http_handler_t* self, char* buffer, int n)
{
    int i;
    int just_in_p;
    int done_p = 0;
...
static
int hh_hdr_parse_block(http_handler_t* self, char* buffer, int n)
{
    int i;
    int just_in_p;
    int done_p = 0;
...
/* identity decoding */
static
int hh_bdy_decode_block(http_handler_t* self, char* buffer, int n)
{
...
static
int hh_bdy_decode_chunked_block(http_handler_t* self, char* buffer, int n)
{
    int done = 0;
    char* readptr = buffer;
...
static
void hh_on_read(transport_callbacks_t* zelf, channel_t* c, char* buffer, int n)
{
    http_handler_t* self = (http_handler_t*) zelf;
...
static
void hh_on_close(transport_callbacks_t* zelf, channel_t* chan)
{
    http_handler_t* self = (http_handler_t*) zelf;
	...
	if(!self->response->connection_close_p && chan->stream.broken_p) {
	    ...
	    if(get_state(self) == BODY) {
		DEBUG3("%d / %d", self->bdy_size, self->response->content_length);
	    }
