 ...
 *
 * You should have received this file ('src/network/transport_callbacks.h') with
 *a license
...
typedef struct transport_callbacks_t {
    ...
    void (*read)(struct transport_callbacks_t *self, channel_t *channel,
     ...
     */
    int (*write)(struct transport_callbacks_t *self, channel_t *channel,
                 char *buffer, int nbytes);

    /* on connection closed, the callback acts as a notification */
    void (*close)(struct transport_callbacks_t *self, channel_t *channel);
