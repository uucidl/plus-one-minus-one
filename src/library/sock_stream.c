 ...
 *
 * You should have received this file ('src/library/sock_stream.c') with a license
...
static
sock_stream_t* sock_stream_open(sock_stream_t* self)
{
...
static
sock_stream_t* sock_stream_set_nonblocking(sock_stream_t* self)
{
    if(!self->error_p) sock_set_nonblocking(self->socket);

    return self->error_p ? NULL : self;
}

static
int sock_stream_close(stream_t* zelf)
{
...
static
int init_sockaddr(struct sockaddr_in* name, url_t* url)
{
...
static
sock_stream_t* sock_stream_connect(sock_stream_t* self, url_t* url)
{
    struct sockaddr_in server_addr;

    self->url.copy(&self->url, url);

    self->error_p = self->error_p ||
	(init_sockaddr(&server_addr, &self->url) < 0);

    if(!self->error_p) {
	int err = sock_connect(self->socket,
...
static
size_t sock_stream_read(void* ptr, size_t size,
			size_t nmemb, struct stream_t* zelf)
{
    sock_stream_t* self = (sock_stream_t*) zelf;
    int n = 0;
...
size_t sock_stream_write(const void* ptr, size_t size,
			 size_t nmemb, stream_t* zelf)
{
    sock_stream_t* self = (sock_stream_t*) zelf;
    int n = 0;
...
int sock_stream_get_errno(sock_stream_t* self)
{
    return errno;
}
