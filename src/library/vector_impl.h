 ...
 *
 * You should have received this file ('src/library/vector_impl.h') with a license
...
static inline
unsigned int get_element_count (vector_t* v) {
	return v->next_element;
}

/* returns pointer to location of element */
static inline 
void*    get_element(vector_t* v, unsigned int i)
{
    if(i < v->next_element) {
...
static inline
void* add_elements (vector_t* v, unsigned int count) {
    void* element;
	...
	memset (v->buffer + old_n * v->element_size, 0, (v->element_n - old_n) * v->element_size);
    }
...
static inline
void*    add_element(vector_t* v)
{
	return add_elements (v, 1);
}
...
/* sets up an iterator */
static inline
void get_iterator(vector_t* v, iterator_t* i)
{
    i->v = v;
    i->i = 0;
