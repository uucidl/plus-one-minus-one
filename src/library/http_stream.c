 ...
 *
 * You should have received this file ('src/library/http_stream.c') with a license
...
static
http_stream_t* http_stream_do(http_stream_t* self, http_method_t method)
{
	...
	if(c && !c->stream.error_p && !response.broken_p) {
		...
		ret = self;
		if(response.size) {
		    memory_open(&self->super, response.data, response.size, "rb");
		    self->super.buffer_owned_p = 1;
		} else if(method == GET) {
		    char* url_s = self->url.create_string(&self->url);
		    ERROR4("size of returned body was zero for get method. location: %s, status: %d, content_length: %d\n", url_s, self->status, self->content_length);
		    free(url_s);
		}
	    } else {
	    ...
	    ERROR5("an error occured while downloading the resource: '%s'\n"
		   ...
		   url_s,
		   !!c, c ? c->stream.error_p : -1, c ? response.broken_p : -1);
	    free(url_s);
	}
...
static
http_stream_t* http_stream_open(http_stream_t* self)
{
    char* s = NULL;
...
static
http_stream_t* http_stream_stat(http_stream_t* self)
{
