 ...
 *
 * You should have received this file ('src/library/stdio_stream.c') with a license
...
static
char* stdio_get_as_memory_area(stream_t* self, int64_t* returned_size)
{
...
static
void stdio_free_memory_area(stream_t* self, char* buffer, int64_t returned_size)
{
    free(buffer);
}

static
size_t stdio_read(void* data, size_t size, size_t nmemb, stream_t* self)
{
    return fread(data, size, nmemb, ((stdio_stream_t*) self)->fd);
}

static
size_t stdio_write(const void* data, size_t size, size_t nmemb, stream_t* self)
{
    return fwrite(data, size, nmemb, ((stdio_stream_t*) self)->fd);
}

static
int stdio_seek(stream_t *self, int64_t offset, int whence)
{
    return fseek(((stdio_stream_t*) self)->fd, offset, whence);
}

static
int stdio_close(stream_t *self)
{
    int ret = 0;
...
static
long stdio_tell(stream_t *self)
{
    return ftell(((stdio_stream_t*) self)->fd);
}

static
int stdio_error_p(stream_t* self)
{
    return ferror(((stdio_stream_t*) self)->fd);
}

static
void stdio_clear_error(stream_t* self)
{
    clearerr(((stdio_stream_t*) self)->fd);
}

static
int stdio_get_length (stream_t* self)
{
...
static
stream_t* stdio_stream_fork (stream_t* zelf)
{
...
stdio_stream_t* stdio_open(stdio_stream_t* x, const char* path, const char* mode)
{
...
static
stdio_stream_t* stdio_stream_from_fd (stdio_stream_t* x, FILE* fd)
{
