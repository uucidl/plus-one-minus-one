 ...
 *
 * You should have received this file ('src/library/memory_stream.c') with a
 *license
...
static char *memory_get_as_memory_area(stream_t *self, int64_t *returned_size)
{
...
static void memory_free_memory_area(stream_t *self, char *buffer,
                                    int64_t returned_size)
{
    // do nothing, it's our internal buffer
}

static size_t memory_read(void *data, size_t size, size_t nmemb,
                          struct stream_t *self)
{
    memory_stream_t *m = (memory_stream_t *)self;
    size_t s = size * nmemb;
...
static int memory_seek(struct stream_t *self, int64_t offset, int whence)
{
    memory_stream_t *m = (memory_stream_t *)self;
    int64_t np;
...
static int memory_close(struct stream_t *self)
{
    memory_stream_t *m = (memory_stream_t *)self;
...
static long memory_tell(struct stream_t *self)
{
    memory_stream_t *m = (memory_stream_t *)self;
...
static int memory_error_p(struct stream_t *self) { return 0; }

static void memory_clear_error(struct stream_t *self)
{
    // nothing
}
...
memory_stream_t *memory_open(memory_stream_t *x, const char *buffer,
                             const int64_t size, const char *mode)
{
...
memory_stream_t *memory_open2(memory_stream_t *x, int buffer_owned_p,
                              const char *buffer, const int64_t size,
                              const char *mode)
{
