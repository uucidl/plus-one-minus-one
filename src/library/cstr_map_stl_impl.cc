 ...
 *
 * You should have received this file ('src/library/cstr_map_stl_impl.cc') with a license
...
typedef HASH_MAP_NS::hash_map<const char*, void*, HASH_MAP_NS::hash<const char*>, eqstr> our_hash_map_t;
...
cstr_map_iterator_t* cstr_map_get_iterator(cstr_map_t* self, cstr_map_iterator_t* iterator) {
    our_hash_map_t* map = (our_hash_map_t*) self->impl;
    cstr_map_iterator_t* it = cstr_map_iterator_instantiate_toplevel (iterator);
    it->map = self;
    iterator_state_t* state = (iterator_state_t*)
	calloc (sizeof (iterator_state_t), 1);
