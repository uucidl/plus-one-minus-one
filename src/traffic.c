...
static const char* sources_url = "zip://virtual://resources/+1-1.dat#sources.text";
		...
		TRACE3("opening %d - %s", arg->spot, url);
		{
			...
			if(!add_to_projector(stream)) {
				...
				if(fd) {
				    ...
				    while(( n = stream_get_callbacks(stream)
					    ->read(data, 1, 4096, stream))) {
					    if (1 != fwrite(data, n, 1, fd)) {
						    ERROR2("couldn't write to %s", filename);
					    }
				    }
				    fclose(fd);
				}
...
void* fetch_google(void* rg)
{
	...
	const char* cquery = "http://images.google.com/images?q=%s+filetype:jpg&svnum=10&hl=en&imgsafe=off&start=0&sa=N&filter=0";
...
void start_grabbing_images(int max_images)
{
	...
	while(stream_get_callbacks(sources)
	      ->read(dest, 1, 1, sources)) {
	    switch(*dest) {
	    case ' ':
		if(status != COMMENT)
		    new_status = SP;
		...
		if(new_status == TYPE) {
		    ...
		    source[index] = '\0';
		    /* starts thread */
		    parsing_threads_ids = realloc(parsing_threads_ids,
						  sizeof(pthread_t)*(thread_count+1));
		    parsing_threads_args = realloc(parsing_threads_args,
			...
			if (is_multithreaded_p) {
			    if(kn_thread_create (&parsing_threads_ids[thread_count], fetching_function, arg)) {
	...
	if (thread_count > 0) {
	    unsigned int* times = calloc
		(thread_count, sizeof (unsigned int));
		...
		for(i=0; i<thread_count; i++) {
		    ...
		    timeout.tv_nsec = (ms - 1000.0*timeout.tv_sec)*1000000.0;
			...
			while(h_block--) {
			    memset(scanline, grey(0x44), width*sizeof(uint32_t));
			    scanline += width;
			}
			demo_get_instance()->update(demo_get_instance(), videobuffer);
		    }
		    pthread_mutex_lock(&parsing_threads_args[i]->complete_mutex);
		    status = parsing_threads_args[i]->finished_p ||
			...
			pthread_mutex_unlock(&parsing_threads_args[i]->complete_mutex);
			pthread_join(parsing_threads_ids[i], NULL);
			if(parsing_threads_args[i]->source)
			    free(parsing_threads_args[i]->source);
			pthread_mutex_destroy(&parsing_threads_args[i]->complete_mutex);
			pthread_cond_destroy(&parsing_threads_args[i]->complete_cond);
			...
			n--;
			{
			    ...
			    amp += times[i];
			    while(h_block--) {
				memset(scanline, grey (amp), width*sizeof(uint32_t));
				scanline += width;
			    }
			    demo_get_instance()->update(demo_get_instance(), videobuffer);
			}
			...
			/* blackens stripe */
			{
			    ...
			    uint32_t* scanline = videobuffer + i * width * h_block;
			    while(h_block--) {
				memset(scanline, grey(0), width*sizeof(uint32_t));
				scanline += width;
			    }
			    demo_get_instance()->update(demo_get_instance(), videobuffer);
			}
