 ...
 *
 * You should have received this file ('src/modplug/modplug_player.c') with a license
...
static
int modplug_player_new(effect_t* self)
{
...
static
int modplug_player_destroy(effect_t* self)
{
    modplug_player_t* p = (modplug_player_t*) self;
...
static
void modplug_player_set_area_parameters(audio_effect_t* self,
					int srate, int frames, int frame_size)
{
    ...
    player->settings.mResamplingMode = MODPLUG_RESAMPLE_LINEAR; //MODPLUG_RESAMPLE_FIR;
...
static
void add_16toSample (sample_t *out, int16_t *in, unsigned long num, float volume)
{
...
static
void modplug_player_computes_area(audio_effect_t* self, audio_area_t* area, double ms)
{
    modplug_player_t* p = (modplug_player_t*) self;
    sample_t* __restrict__ samples = area->samples + area->head * p->super.super.frame_size;
...
static
void modplug_player_eom(void* zelf, double ms)
{
...
static
int modplug_player_load_stream(resource_audio_effect_t* zelf, stream_t* stream, progress_t* progress)
{
...
static
void modplug_player_set_master_volume(resource_audio_effect_t* zelf, const float volume, context_t* c)
{
    modplug_player_t* self = (modplug_player_t*) zelf;
    if(self->running_p)
...
static
void modplug_player_set_row_callback(modplug_player_t* self,
...
static
void modplug_player_set_looping_mode(resource_audio_effect_t* zelf, atom_t looping_p, context_t* c)
{
    modplug_player_t* self = (modplug_player_t*) zelf;
...
static
void modplug_player_get_music_speed(modplug_player_t* self,
