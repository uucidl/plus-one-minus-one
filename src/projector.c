...
static int projector_destroy(effect_t *zelf)
{
...
static source_t *add_source(projector_t *self)
{
    if (self->next_source == self->sources_n) {
        int old_n = self->sources_n;
        self->sources_n += 1;
        self->sources =
            realloc(self->sources, sizeof(source_t) * self->sources_n);
        memset(self->sources + old_n, 0,
               sizeof(source_t) * (self->sources_n - old_n));
    }
...
static int projector_add_image(projector_t *self, stream_t *fd)
{
    image_t *x;
...
static void projector_set_current(projector_t *self, int i)
{
...
static void projector_computes(effect_t *self, void *content, double ms)
{
    projector_t *proj = (projector_t *)self;
    ...
    if (image->pixels && image->width && image->height) {
        ...
        */
        ...
        if (do_not_scale_p) {
            image_blit_blend(image, x / 2, y / 2, pixels, proj->super.width,
                             proj->super.height, proj->super.pitch);
        } else {
            image_blit_blend_scale(image, source->x / 2, source->y / 2,
                                   source->zoom, source->zoom, pixels,
                                   proj->super.width, proj->super.height,
                                   proj->super.pitch);
        }
    }
}

static void projector_set_normalized_magnification(projector_t *self, int i,
                                                   float z)
{
    source_t *source = NULL;
    image_t *image = NULL;
