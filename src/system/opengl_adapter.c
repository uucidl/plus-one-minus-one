 ...
 *
 * You should have received this file ('src/system/opengl_adapter.c') with a
 *license
...
static int opengl_adapter_destroy(effect_t *zelf)
{
...
static int opengl_adapter_plug_effect(opengl_adapter_t *self, effect_t *e,
                                      int own_p)
{
    int status;
    ...
    if (video_frame_atom == frame_type) {
        ...
        self->fc =
            &video_to_opengl_frame_converter_instantiate_toplevel(NULL)->super;
        self->fc->new (self->fc);
...
static void opengl_adapter_computes(effect_t *zelf, void *content, double ms)
{
    opengl_adapter_t *self = (opengl_adapter_t *)zelf;
