 ...
 *
 * You should have received this file ('src/system/audio_effect.c') with a license
...
audio_area_audio_frame_signature_t* audio_area_audio_frame_signature_instantiate (audio_area_audio_frame_signature_t*  x) {
...
static
int audio_effect_new(effect_t* self)
{
    return 1;
}

static
int audio_effect_destroy(effect_t* self)
{
    return 1;
}

static
void silence_computes_area(audio_effect_t* self, audio_area_t* area, double ms)
{
    int len = self->frame_size*area->frame_number;
    sample_t* __restrict__ samples = area->samples + area->head*self->frame_size;
    int i; 
...
static 
void audio_effect_computes(effect_t* self, void* content, double ms)
{
...
static
void audio_effect_computes_in_parts(effect_t* self, void* content, double ms)
{
    audio_effect_t* x = (audio_effect_t*) self;
	...
	/* determine when next event will happen */
	{
	    ...
	    next_event_ms = ms + 1000.0*(area_in->frame_number+1)/x->sample_rate; 
	    c.ms = next_event_ms;
	...
	if(next_event_ms < c.ms) {
	    area_out.frame_number = (next_event_ms-now)*x->sample_rate / 1000.0;
	    area_out.frame_number+= 1;
	    /* clip it */
	    area_out.frame_number = area_out.frame_number > count ? 
...
static
void audio_effect_set_area_parameters(audio_effect_t* self, int srate, int frames, int frame_size)
{
...
static
atom_t audio_effect_get_frame_type(effect_t* self)
{
    return frame_type;
}

static
void audio_effect_set_frame_signature(effect_t* self, object_t* signature)
{
    audio_effect_t* ae = (audio_effect_t*) self;
    audio_area_audio_frame_signature_t* sig = (audio_area_audio_frame_signature_t*) signature;
    ae->set_area_parameters(ae, sig->sample_rate, sig->max_frame_number, sig->frame_size);
}
...
audio_effect_t* audio_effect_instantiate(audio_effect_t* x)
{
    ...
    if(!frame_type) {
	frame_type = dictionary_get_instance()->new_atom(dictionary_get_instance(), AUDIO_EFFECT_FRAME_TYPE_NAME);
    }
...
static 
void audio_silence_computes(effect_t* self, void* content, double ms)
{
...
static
void audio_silence_set_area_parameters(audio_effect_t* self, 
				       int srate, int frames, int frame_size)
{
    audio_silence_t* as = (audio_silence_t*) self;
    audio_effect_set_area_parameters(self, srate, frames, frame_size);
    
    if(as->next) {
	as->next->set_area_parameters(as->next, 
				      as->super.sample_rate, 
...
static
void audio_silence_set_next(audio_silence_t* self, audio_effect_t* next)
{
    self->next = next;
    if(next && 
       self->super.sample_rate && self->super.frame_number && self->super.frame_size) {
	self->next->set_area_parameters(self->next, 
