...
static
int null_kgo_driver_new(kgo_driver_t* self, 
...
static
int null_kgo_driver_destroy(struct kgo_driver_t* self)
{
    return 1;
}

static
int null_kgo_driver_has_fd(struct kgo_driver_t* self)
{
    return 0;
}

static
int null_kgo_driver_get_fd(struct kgo_driver_t* self)
{
    return -1;
}

static
event_listener_t* null_kgo_driver_get_event_listener(struct kgo_driver_t* self)
{
    return NULL;
}

static
void null_kgo_driver_update_frame(struct kgo_driver_t* self, void* buffer, int event_pending)
{
    // do nothing
}

static
atom_t null_kgo_driver_get_frame_type(kgo_driver_t* self)
{
    return dictionary_get_instance()->new_atom(dictionary_get_instance(), VIDEO_EFFECT_FRAME_TYPE_NAME);
}

static
void* null_kgo_driver_allocate_frame(kgo_driver_t* self)
{
    return NULL;
}

static 
int video_effect_kgo_driver_configure_demo(kgo_driver_t* self, demo_t* d)
{
    int error_p = 0;
    ...
    if (!error_p) {
	atom_t frame_type = d->kgo_effect_root->get_frame_type (d->kgo_effect_root);
	dictionary_t* dict = dictionary_get_instance ();
	
	error_p = (frame_type != dict->get_atom (dict, VIDEO_EFFECT_FRAME_TYPE_NAME));

	if (error_p) {
	    ERROR3("video effect has invalid frame type: %s. (expected %s)", 
		   atom_get_cstring_value (frame_type), VIDEO_EFFECT_FRAME_TYPE_NAME);
	}
    }
	...
	d->kgo_effect_root->set_frame_signature
	    (d->kgo_effect_root, &frame_signature.super);
...
static
void kgo_null_kgo_driver_initialize() __attribute__((constructor));

static
void kgo_null_kgo_driver_initialize()
{
