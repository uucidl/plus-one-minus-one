...
static void initialize_driver() { driver = null_pan_driver_instantiate(); }
...
/*
  start device
*/
void pan_start() { driver->start(driver); }

/*
  stop playing
*/
void pan_stop() { driver->stop(driver); }

int pan_close() { return driver->destroy(driver); }

int pan_get_samples_number() { return driver->get_samples_number(driver); }

int pan_get_sample_rate() { return driver->get_sample_rate(driver); }

long int pan_get_fd() { return driver->get_fd(driver); }

int pan_is_ready() { return driver->is_ready(driver); }

void pan_update(sample_t *samples) { driver->update(driver, samples); }

double pan_gettime() { return driver->get_time(driver); }
