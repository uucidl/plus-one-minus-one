 ...
 *
 * You should have received this file ('src/system/video_adapter.c') with a license
...
static
int video_adapter_destroy(effect_t* zelf)
{
...
static
int video_adapter_plug_effect(video_adapter_t* self, effect_t* e, int own_p)
{
    int status;
    atom_t video_frame_atom = dictionary_get_instance()->get_atom(dictionary_get_instance(), OPENGL_FRAME_TYPE_NAME);
    atom_t frame_type = e->get_frame_type(e);

    if(video_frame_atom == frame_type) {
	INFO1("all is fine and dandy.");
	self->fc = &opengl_to_video_frame_converter_instantiate_toplevel(NULL)->super;
	self->fc->new(self->fc);
...
static
void video_adapter_computes(effect_t* zelf, void* content, double ms)
{
    video_adapter_t* self = (video_adapter_t*) zelf;
