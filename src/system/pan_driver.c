...
static
int pan_driver_configure_demo(pan_driver_t* p, demo_t* demo)
{
	...
	/* check frame type */
	dictionary_t* dict = dictionary_get_instance ();
	atom_t frame_type = demo->pan_effect_root->get_frame_type (demo->pan_effect_root);

	if (frame_type != dict->get_atom (dict, AUDIO_EFFECT_FRAME_TYPE_NAME)) {
	    ERROR3 ("Invalid frame type for audio effect. (%s) Expected %s.",
		    atom_get_cstring_value (frame_type), AUDIO_EFFECT_FRAME_TYPE_NAME);
	    return 0;
	}
    }
	...
	demo->pan_effect_root->set_frame_signature (demo->pan_effect_root, &signature.super);
...
static
int null_pan_driver_new(pan_driver_t* zelf, const char* device, int sample_rate)
{
...
static
int null_pan_driver_destroy(pan_driver_t* self)
{
    return 1;
}

static
void null_pan_driver_start(pan_driver_t* zelf)
{
    null_driver_t* self = (null_driver_t*) zelf;
...
static
void null_pan_driver_stop(pan_driver_t* zelf)
{
    null_driver_t* self = (null_driver_t*) zelf;
...
static
void null_pan_driver_update(pan_driver_t* self, sample_t* samples)
{
    // nothing to be done
}

static
int null_pan_driver_get_sample_rate(pan_driver_t* zelf)
{
   null_driver_t* self = (null_driver_t*) zelf;
...
static
int null_pan_driver_get_samples_number(pan_driver_t* zelf)
{
   return 32;
}

static
int null_pan_driver_return0(pan_driver_t* self)
{
    return 0;
}

static
long int null_pan_driver_returnlong0(pan_driver_t* self)
{
    return 0;
}

static
double null_pan_driver_get_time(pan_driver_t* zelf)
{
    null_driver_t* self = (null_driver_t*) zelf;
...
static
void pan_null_pan_driver_initialize() __attribute__ ((constructor));

static
void pan_null_pan_driver_initialize()
{
