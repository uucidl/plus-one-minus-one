...
static 
int core_effect_new(effect_t* this)
{
    // nothing
    return 1;
}
...
static 
int core_effect_destroy(effect_t* this)
{
    return 1;
}

static
void core_effect_computes(effect_t* this, void* frame, double ms)
{
    // does nothing
}

static
void core_effect_set_mode(effect_t* this, emode_t mode)
{
    switch(mode)
    {
    case ON:
	...
	this->mode = ON;
	TRACE2("'%s' setting mode: ON", atom_get_cstring_value(this->class->class));
	break;
    case OFF:
	if(this->mode == ON) {
	...
	this->mode = OFF;
	TRACE2("'%s' setting mode: OFF", atom_get_cstring_value(this->class->class));
	break;
    }
}

static
void core_effect_send(effect_t* self, const char* message)
{
    context_t* c;
    bytecode_stream_t* m;
...
static
void core_effect_send_msg(effect_t* self, 
			  bytecode_stream_t* m, 
			  context_t* c)
{
...
*/
static
double core_effect_get_latency_ms(effect_t* this)
{
    return 0.;
}

static
atom_t core_effect_get_frame_type(effect_t* this)
{
...
static
void core_effect_set_frame_signature(effect_t* this, object_t* signature)
{
    /* nothing to do */
}
...
effect_t* effect_instantiate(effect_t* x)
{
	...
	r->set_definition(r, d->new_atom(d, "set-mode"), compile_cstring("integer", NULL), (recp_f) e->set_mode);
...
void effect_register_instance(const char* class_name, effect_t* instance)
{
    ...
    instance->id    = 
	effect_get_class(instance)->attach_instance
