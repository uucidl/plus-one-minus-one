 ...
 *
 * You should have received this file ('src/system/audio_effect.h') with a license
...
typedef struct audio_area_audio_frame_signature_t
{
    ...
    int max_frame_number; /* maximum number of sampling frames in each effect frame */
...
{
    effect_t super;
    void (*set_area_parameters)(struct audio_effect_t* self,
				int sample_rate, int frame_number, int frame_size);
    /*
       by default, audio_effect_t::effect::computes cuts the input area into as many
       areas necessary to handle each received events at their ideal time, calling
       this method for each area.

       overriding audio_effect_t::effect::computes removes this behaviour.
    */
    void (*computes_area)(struct audio_effect_t* self,
...
/*
  some example audio_effect that first clears the area with silence, then calls the 'next effect'
*/
