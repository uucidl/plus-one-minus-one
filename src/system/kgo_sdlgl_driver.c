...
static
void setupWIN32 (kgo_sdlgl_driver_t* self)
{
  int r;
  SDL_SysWMinfo info;
  HWND hWnd;
...
static
Display* kgo_sdlgl_get_display (kgo_driver_t* zelf)
{
...
static
Window kgo_sdlgl_get_window (kgo_driver_t* zelf)
{
    kgo_sdlgl_driver_t* self = (kgo_sdlgl_driver_t*) zelf;
...
static
void setupMACOSX(kgo_sdlgl_driver_t* self) {
...
static
int kgo_sdlgl_driver_new(kgo_driver_t* zelf,
			 char* title, unsigned int width, unsigned int height)
{
	...
	if(ok) {
	    ...
	    */
	    ...
	    ok = !!SDL_SetVideoMode(self->window_width, self->window_height, bpp, flags);
...
static
int kgo_sdlgl_driver_destroy(struct kgo_driver_t* self)
{
    SDL_ShowCursor(1);
    SDL_Quit ();
...
static
int kgo_sdlgl_driver_has_fd(struct kgo_driver_t* self)
{
#ifdef LINUX
    return 0;
#else
...
static
int kgo_sdlgl_driver_get_fd(struct kgo_driver_t* self)
{
    return -1;
}
...
static
sdl_event_listener_t* event_listener = NULL;

static
event_listener_t* kgo_sdlgl_driver_get_event_listener(struct kgo_driver_t* zelf)
{
    kgo_sdlgl_driver_t* self = (kgo_sdlgl_driver_t*) zelf;
    if(!event_listener) {
	event_listener = sdl_event_listener_instantiate_toplevel (NULL);
	if(self->windowed_p && (self->window_width != self->width || self->window_height != self->height)) {
	    event_listener->set_mouse_motion_correction
		(event_listener, 1. * self->width / self->window_width, 1. * self->height / self->window_height);
	}
    }
...
static
int process_events(kgo_driver_t* zelf, int event_pending)
{
    kgo_sdlgl_driver_t* self = (kgo_sdlgl_driver_t*) zelf;
    SDL_Event sdl_event;
...
static
void kgo_sdlgl_driver_update(struct kgo_driver_t* zelf, void* buffer, int event_pending)
{
    kgo_sdlgl_driver_t* self = (kgo_sdlgl_driver_t*) zelf;

    if (buffer != NULL && self->has_adapter_p) {
	    opengl_adapter_t* adapter = (opengl_adapter_t*) self->demo->kgo_effect_root;
...
static
atom_t kgo_sdlgl_driver_get_frame_type(struct kgo_driver_t* zelf)
{
    return dictionary_get_instance()->new_atom(dictionary_get_instance(), OPENGL_FRAME_TYPE_NAME);
}

static
void* kgo_sdlgl_driver_allocate_frame(struct kgo_driver_t* zelf)
{
    return NULL;
}

static
int kgo_sdlgl_driver_configure_demo(kgo_driver_t* zelf, demo_t* demo)
{
    kgo_sdlgl_driver_t* self = (kgo_sdlgl_driver_t*) zelf;
    self->demo = demo;
	...
	if (frame_type == dict->get_atom (dict, VIDEO_EFFECT_FRAME_TYPE_NAME)) {
	    ...
	    opengl_adapter_t* adapter = opengl_adapter_instantiate_toplevel (NULL);
	    ...
	    ERROR2 ("video effect has invalid frame type: %s", atom_get_cstring_value (frame_type));
	    error_p = 1;
	}

	/* for now, no geometry info */
	{
		demo->kgo_effect_root->set_frame_signature
			(demo->kgo_effect_root, NULL);
	}
...
kgo_sdlgl_driver_t* kgo_sdlgl_driver_instantiate(kgo_sdlgl_driver_t* x)
{
    ...
    sdlgl_driver->super.get_event_listener = kgo_sdlgl_driver_get_event_listener;
...
{
    kgo_sdlgl_driver_t* windowed_driver	  = kgo_sdlgl_driver_instantiate_toplevel(NULL);
    kgo_sdlgl_driver_t* fullscreen_driver = kgo_sdlgl_driver_instantiate_toplevel(NULL);
