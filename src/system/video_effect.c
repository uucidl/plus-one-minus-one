 ...
 *
 * You should have received this file ('src/system/video_effect.c') with a license
...
argb32_video_frame_signature_t* argb32_video_frame_signature_instantiate (argb32_video_frame_signature_t* x) {
...
static
int blank_new(effect_t* self)
{
    return 1;
}

static
int blank_destroy(effect_t* self)
{
    return 1;
}

static 
void blank_computes(effect_t* self, void* content, double ms)
{
    video_effect_t* x = (video_effect_t*) self;
    int h = x->height;
    while(h--) {
	memset 
		(content + h * x->pitch * sizeof(int32_t), 
		 0x00, 
...
static
void set_frame_size_v(video_effect_t* self, int width, int height, int pitch)
{
    self->width  = width;
    self->height = height;
    self->pitch  = pitch;
...
static
atom_t get_frame_type_v(effect_t* self)
{
    return frame_type;
}

static
void set_frame_signature_v(effect_t* self, object_t* signature)
{
    video_effect_t* ve = (video_effect_t*) self;
    argb32_video_frame_signature_t* sig = (argb32_video_frame_signature_t*) signature;
...
video_effect_t* video_effect_instantiate(video_effect_t* x)
{
    ...
    if(!frame_type) {
	frame_type = dictionary_get_instance()->new_atom(dictionary_get_instance(), VIDEO_EFFECT_FRAME_TYPE_NAME);
    }
...
static 
void video_blank_computes(effect_t* self, void* content, double ms)
{
...
static
void video_blank_set_frame_size(video_effect_t* self, int width, int height, int pitch)
{
    video_blank_t* vb = (video_blank_t*) self;
...
static
void video_blank_set_next(video_blank_t* self, video_effect_t* next)
{
    self->next = next;
    if(next) {
	self->next->set_frame_size(self->next, 
				   self->super.width, 
				   self->super.height,
				   self->super.pitch);
    }
