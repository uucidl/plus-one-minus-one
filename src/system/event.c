    ...
    if (a == b) {
        ...
        if (!a || !b)
            ...
            if (a->size != b->size)
                ...
                return memcmp(a->hierarchy, b->hierarchy,
                              a->size * sizeof(bytecode_t)) == 0;
            }
        }
    }
}
...
event_t *event_copy(const event_t *self, event_t *to)
{
    if (self == NULL)
        ...
        memcpy(other->hierarchy, self->hierarchy,
               sizeof(bytecode_t) * other->size);
...
static int event_release(object_t *o)
{
    event_t *e = (event_t *)o;
