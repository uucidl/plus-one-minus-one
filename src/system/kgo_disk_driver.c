 ...
 *
 * You should have received this file ('src/system/kgo_disk_driver.c') with a
 *license
                               ...
                               unsigned int width, unsigned int height)
{
    ...
    {
        const unsigned int n = strlen("%s-%0") +
                               (1 + (int)log10(self->number_n)) +
                               strlen("d.tga") + 1;
...
static int kgo_disk_driver_destroy(struct kgo_driver_t *zelf)
{
    kgo_disk_driver_t *self = (kgo_disk_driver_t *)zelf;
...
static int kgo_disk_driver_configure_demo(kgo_driver_t *zelf, demo_t *demo)
{
    kgo_disk_driver_t *self = (kgo_disk_driver_t *)zelf;
    ...
    if (!error_p) {
        ...
        if (frame_type == opengl_frame_type) {
            ...
            self->subdriver->new (self->subdriver, self->title,
                                  self->image.width, self->image.height);
            self->subdriver->configure_demo(self->subdriver, demo);
            self->subdriver_frame =
                self->subdriver->allocate_frame(self->subdriver);
...
static void kgo_disk_driver_start(struct kgo_driver_t *zelf)
{
    kgo_disk_driver_t *self = (kgo_disk_driver_t *)zelf;
...
static void kgo_disk_driver_stop(struct kgo_driver_t *zelf)
{
    kgo_disk_driver_t *self = (kgo_disk_driver_t *)zelf;
...
static int kgo_disk_driver_has_fd(struct kgo_driver_t *self) { return 0; }

static int kgo_disk_driver_get_fd(struct kgo_driver_t *self) { return -1; }

static event_listener_t *
kgo_disk_driver_get_event_listener(struct kgo_driver_t *self)
{
    return event_listener_instantiate_toplevel(NULL);
}

void kgo_disk_driver_update_frame(struct kgo_driver_t *zelf, void *frame,
                                  int event_pending)
{
    kgo_disk_driver_t *self = (kgo_disk_driver_t *)zelf;
    ...
    if (self->started_p) {
        if (self->subdriver) {
            self->subdriver->update_frame(self->subdriver,
                                          self->subdriver_frame, event_pending);
        }

        int j;
        for (j = 0; j < self->image.height; j++)
            memcpy(self->image.pixels + j * self->image.pitch,
                   source + j * self->image.width,
                   self->image.width * sizeof(int32_t));
        {
            FILE *fd;
            snprintf(self->filename, self->filename_n, self->format,
                     self->title, self->frame);
...
static atom_t kgo_disk_driver_get_frame_type(struct kgo_driver_t *zelf)
{
    return dictionary_get_instance()->new_atom(dictionary_get_instance(),
                                               VIDEO_EFFECT_FRAME_TYPE_NAME);
}

static void *kgo_disk_driver_allocate_frame(struct kgo_driver_t *zelf)
{
    kgo_disk_driver_t *self = (kgo_disk_driver_t *)zelf;
...
static void kgo_disk_driver_initialize() __attribute__((constructor));

static void kgo_disk_driver_initialize()
{
