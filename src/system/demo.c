...
static
void demo_set_unlimited_duration (struct demo_t* self) {
	self->end_ms = -1;
}

static
void demo_tick (struct demo_t* self, const double timeMs) {
...
static
void demo_update(demo_t* self, uint32_t* videobuffer)
{
    if(videobuffer)
...
static
void demo_send(demo_t* self, const char* message)
{
    context_t* c = NULL;
    bytecode_stream_t* m;
...
static
void demo_send_msg(demo_t* self,
		   bytecode_stream_t* m, context_t* c)
{
...
static
void demo_set_video_effect (demo_t* self, effect_t* e)
{
...
static
void demo_set_audio_effect (demo_t* self, effect_t* e)
{
    self->pan_effect_root = e;
}

static
void demo_set_gui_event_listener (demo_t* self, event_listener_t* e)
{
...
static
void demo_set_midi_event_listener (demo_t* self, event_listener_t* e)
{
    if (self->midi_listener != e) {
	self->midi_listener = e;
	self->midi_listener->add_child (self->midi_listener, &self->event_filter);
    }
}
...
demo_t* demo_instantiate(demo_t* x)
{
	...
	d->event_filter.signature.super.hierarchy[0].adverb = dict->get_atom (dict, "atom");
	d->event_filter.signature.super.hierarchy[0].verb   = dict->get_atom (dict, "*");
    }
