...
static void rub_computes(effect_t *self, void *content, double ms)
{
...
static void rub_masked_computes(effect_t *self, void *content, double ms)
{
    blur_t *blur = (blur_t *)self;
    int32_t *__restrict__ pixels = content;
    int32_t *__restrict__ scanline;
...
static void rub_set_mode(blur_t *self, enum blur_mode_t mode)
{
    if (mode == MASKED)
