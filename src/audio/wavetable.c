...
static
void wavetable_precompute(wavetable_t w)
{
    int i;
    for (i=0; i<WAVETABLE_SIZE; i++)
	w[i].slope = 
	    (w[(i + 1) & (WAVETABLE_SIZE - 1)].value - 
	     w[i].value);
...
wavetable_t wavetable_get_exponential_1()
{
	...
	for (i=0; i<WAVETABLE_SIZE; i++)
	    exponential_1[i].value = 1.0 / M_E * 
		exp( -1.0*( target*i/WAVETABLE_SIZE ) );
...
wavetable_t wavetable_get_triangle()
{
	...
	for(; i<3*WAVETABLE_SIZE/4; i++) {
	    triangle[i].value = 1.0f - 
		(1.0f*(i-WAVETABLE_SIZE/4)/(WAVETABLE_SIZE/4));
	}
	for(; i<WAVETABLE_SIZE; i++) {
	    triangle[i].value = 1.0f*(i-3*WAVETABLE_SIZE/4)/(WAVETABLE_SIZE/4) - 1.0f;
	}
