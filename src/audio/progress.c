...
static
state_t progress_get_state (progress_t* self)
{
    return atomic_intptr_get (&self->status);
}

static
maybe_size_t progress_get_max_n (progress_t* self)
{
    maybe_size_t result;
...
static
maybe_size_t progress_get_current_n (progress_t* self)
{
    maybe_size_t result;
...
static
void progress_set_state (progress_t* self, state_t st)
{
...
static
void progress_set_max_n (progress_t* self, maybe_size_t n)
{
...
static
void progress_set_current_n (progress_t* self, maybe_size_t n)
{
    int nothing_p = MAYBE_IS_NOTHING (&n);
	...
	atomic_intptr_set (MAYBE_REF_LVALUE (&self->current_n), MAYBE_GET_VALUE (&n));
    }
}

static
void progress_advance_current_n(progress_t* self, size_t by)
{
