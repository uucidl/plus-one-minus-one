 ...
 *
 * You should have received this file ('src/audio/resource_audio_effect.h') with a license
...
/*
  an (abstract) effect dedicated to compute/generate the samples taken from a disk resource
*/
typedef struct resource_audio_effect_t
{
    ...
    /* 
       this function is guaranteed not to be realtime safe. Don't call it inside computes! 

       loading progress is written to the preallocated progress structure. (which if NULL is discarded)
    */
    int (*load_file)(struct resource_audio_effect_t* self, stream_t* file, progress_t* progress);
    /* set looping mode if input is true. looping mode may be altered
       if the resource is not loopable */
    void (*set_looping_mode)(struct resource_audio_effect_t* self, const atom_t loop_p, context_t* c);
    /* master volume, [0.0f, 1.0f] */
    void (*set_master_volume)(struct resource_audio_effect_t* self, const float volume, context_t* c);
    ...
    */
    struct resource_audio_effect_t* (*fork) (struct resource_audio_effect_t* self);
