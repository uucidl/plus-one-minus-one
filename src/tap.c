...
static
void tapin_set_size(tapin_t* self, unsigned int size)
{
  if(size) {
...
static
void tapin_computes_area(audio_effect_t* e, audio_area_t* area, double ms)
{
  ...
  if(self->line) {
    /* fill the circular buffer */
	  ...
	  self->line[2*self->line_phase] = samples[i*self->super.frame_size];
	  self->line[2*self->line_phase+1] = samples[i*self->super.frame_size+1];
...
static
void tapout_wraparound(tapout_t* self)
{
  if(self->tapin) {
...
static
void tapout_set_tapin(tapout_t* self, tapin_t* tapin)
{
  self->tapin = tapin;
...
static
void tapout_set_offset(tapout_t* self, unsigned int offset)
{
...
static
void tapout_computes_area(audio_effect_t* e, audio_area_t* area, double ms)
{
  ...
  if(self->tapin) {
    /* fill the circular buffer */
      for(i=0; i < area->frame_number; i++) {
	  samples[self->super.frame_size*i] = self->tapin->line[2*self->out_phase];
	  samples[self->super.frame_size*i+1] = self->tapin->line[2*self->out_phase+1];
