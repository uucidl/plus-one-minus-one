...
static char* font_url   = "zip://virtual://resources/+1-1.dat#RopaSans-Regular.otf"
#else
...
static
void rub_callback(void* self, int length, context_t* context)
{
    if(context)
      rub_ms = context->ms;
    rub_length = length;
...
static
void ice_callback(void* self, context_t* context)
{
    if(context)
      ice_ms = context->ms;
    else
...
static
void ice_alpha_callback(icefx_t* self, unsigned int alpha, context_t* context)
{
	self->alpha = alpha;
}
...
static
void switch_stutter(event_listener_t* self, const event_t* e)
{
...
typedef struct stutter_control_t
{
    ...
    int pressed_p;
    enum { SC_YNONE, SC_HIGH_BASE,
	   SC_LOW_BASE, SC_FB } controlled_variableX;
...
static
void stutter_control_setY(stutter_control_t* self, double adjustment)
{
...
static
void stutter_control_setX(stutter_control_t* self, double adjustment)
{
...
static
void stutter_control_accept(event_listener_t* zelf, const event_t* e)
{
	...
	if(event->hierarchy[2].verb == dict->get_atom(dict, "up")) {
	    ...
	    } else if(event->hierarchy[3].verb == dict->get_atom(dict, "rmb")) {
		self->controlled_variableY = SC_HIGH_BASE;
		self->controlled_variableX = SC_SPEED;
	    } else if(event->hierarchy[3].verb == dict->get_atom(dict, "mmb")) {
	...
	if(self->pressed_p) {
		...
		double variation_y =
		    1.0*atom_get_integer_value(event->hierarchy[3].verb) - self->y0;
		double variation_x =
		    1.0*atom_get_integer_value(event->hierarchy[2].verb) - self->x0;
...
void event_listeners_setup()
{
	 ...
	 fl_space_key->signature.super.hierarchy[1].adverb = dict->get_atom(dict, "atom");
	 fl_space_key->signature.super.hierarchy[1].verb = dict->get_atom(dict, "?");
	 fl_space_key->signature.super.hierarchy[2].adverb = dict->get_atom(dict, "atom");
	 fl_space_key->signature.super.hierarchy[2].verb = dict->get_atom(dict, "?");
	 fl_space_key->signature.super.hierarchy[3].adverb = dict->get_atom(dict, "integer");
	 ...
	 demo_get_instance()->ptc_listener->add_child(demo_get_instance()->ptc_listener,
						      fl_space_key);
     }
	 ...
	 {
	     filtered_listener_t* fl_mouse_button = malloc(sizeof(filtered_listener_t));
		 ...
		 (demo_get_instance()->ptc_listener,
		  fl_mouse_button);
	 }

	 /* mouse motion control */
	 {
	     filtered_listener_t* fl_mouse_motion = malloc(sizeof(filtered_listener_t));
...
void receiver_setup()
{
    ...
    receiver->set_definition(receiver, rub_atom,   compile_cstring("integer", NULL), (recp_f) rub_callback);
    receiver->close(receiver, rub_atom);
    receiver->set_definition(receiver, ice_atom,   compile_cstring("", NULL),        (recp_f) ice_callback);
    receiver->set_definition(receiver, alpha_atom, compile_cstring("integer", NULL), (recp_f) ice_alpha_callback);
}

static
void modplug_callback(void* self, double ms,
		      unsigned int chn,
		      unsigned char note,
		      unsigned char instr,
...
{
    const double diffms = ms + stutter->super.super.get_latency_ms(&stutter->super.super);
...
int bidule_new(effect_t* self)
{
    ...
    text_display->ft.use_font(&text_display->ft, text_display->ft.load_font
			      (&text_display->ft,
			       url_open(font_url, "rb")));
...
void bidule_set_frame_size(video_effect_t* self, int width, int height, int pitch)
{
    self->width = width;
    self->height = height;
    self->pitch = pitch;
    ...
    ((projector_t*)tv)->set_normalized_magnification((projector_t*)tv, 1, 1.0f);
...
void bidule_compute(effect_t* e, void* content, double ms)
{
    ...
    if(ms < 80000.0) {
      if(ms < 8000.0) {
	fade_in_ms = ms;
	((vlogic_t*)vlogic)->generate_palette
	    ((vlogic_t*)vlogic, fade_in_palette);
      }
      ((vlogic_t*)vlogic)->set_multiplier
	  ((vlogic_t*)vlogic, 1, 1.0/(9.0+8.0*sin(pfq*ms)));
    } else
	((vlogic_t*)vlogic)->set_multiplier
	    ((vlogic_t*)vlogic, 1, 1.0/ceil(9.0+8.0*sin(pfq*ms)));
	...
	metro_event_t* e = ((beatsource_t*) metro)->pump((beatsource_t*) metro, ms);
	...
	if (occured_p)
	    ((effect_t*)text_display)->computes((effect_t*)text_display, content, ms);
    }
	...
	if(ice_ms >= 0.0 && fabs(ms-ice_ms) <= 2*demo_get_instance()->video_frame_ms) {
	...
	if(rub_ms >= 0.0 && fabs (ms-rub_ms) <= 80.0) {
		...
		((projector_t*)projector)->set_current ((projector_t*)projector, i_id);
		blur_length = 8;
	    } else {
		blur_length = rub_length;
		rub_p = 1;
	    }
	} else if(blur_ms >= 0.0 && fabs (ms-blur_ms) <= 400.0*ms*ms / (demo_get_instance()->end_ms * demo_get_instance()->end_ms)) {
	    blur_p = 1;
	...
	if(ms >= zoom_ms && (ms-zoom_ms) <= 120.0) {
	    ...
	    ((projector_t*)projector)->set_normalized_magnification
	      ((projector_t*)projector, i_id, zoom);
	} else {
...
video_effect_t* bidule_instantiate()
{
    ...
    tv->new(tv);
    ((projector_t*)tv)->add_image
	((projector_t*)tv, url_open(frame_url, "rb"));
...
static
void truc_set_area_parameters(audio_effect_t* a,
			      int sample_rate, int frame_number, int frame_size)
{
    ...
    silencer->super.set_area_parameters(&silencer->super,
					sample_rate, frame_number, frame_size);
    modplug->super.super.set_area_parameters(&modplug->super.super,
				       sample_rate, frame_number, frame_size);
    alogic->super.set_area_parameters(&alogic->super,
				      sample_rate, frame_number, frame_size);
    stutter->super.set_area_parameters(&stutter->super,
				       sample_rate, frame_number, frame_size);
    tapin->super.set_area_parameters(&tapin->super,
				     sample_rate, frame_number, frame_size);
    tapout->super.set_area_parameters(&tapout->super,
				      sample_rate, frame_number, frame_size);
...
static
void truc_computes_area(audio_effect_t* self, audio_area_t* area, double ms)
{
  ...
  proba2 = (sin(sin(ms/20000.0)) + cos(cos(ms/20000.0)) + sin(ms/103000.0))/3.0;

  {
	  ...
	  if(flag_1_p) {
	      ...
	      high_base = 1.5;
	      if(stutter_ui) {
		  stutter_ui->low_gauge->set_percent(stutter_ui->low_gauge, low_base);
		  stutter_ui->high_gauge->set_percent(stutter_ui->high_gauge, high_base);
	      }
	      flag_1_p = 0;
	  }
	  ...
	  if(p > 0)
	      high_limit = 65536.0*1000.0 / (self->sample_rate * p) - low_base;

	  samples = 1.0 * self->sample_rate/1000.0 * p * (low_base + high_base*exprand());
	  ...
	  stutter->set_block_size(stutter, samples);
	  {
		...
		bytecode_stream_instantiate_toplevel(NULL);
	      ...
	      stream->append_integer(stream, 256*stutter->block_size / 65536);
	      stream->append_atom(stream, rub_atom);

	      c->object = NULL;
	      /* latency of stutter object is roughly equal to block_size */
	      c->ms = ms + stutter->super.super.get_latency_ms(&stutter->super.super);
	      receiver->receive(receiver, stream, c);
	  }
      }
  }
  ...
  /* repeat probability */
  {
	  ...
	  repeat_proba = 0.0;
      if(stutter_ui)
	  stutter_ui->repeat_gauge->set_percent(stutter_ui->repeat_gauge, repeat_proba);
  ...
  stutter->super.super.computes((effect_t*) stutter, area, ms);
  {
	  ...
	  // fb: 0.58f
	  samples[i*self->frame_size] += fb*buffer[self->frame_size*i+1];
	  samples[i*self->frame_size+1] += fb*buffer[self->frame_size*i];
      }
...
audio_effect_t* truc_instantiate()
{
  ...
  if (!modplug->super.load_file(&modplug->super, url_open(module_url, "rb"), NULL)) {
