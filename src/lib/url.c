...
static
int url_open_with_protocol(url_t* self, const char* protocol, const char* url)
{
...
static
int url_new(url_t* self, const char* url)
{
    /* parts of an url */
    const char* resource;
    char* protocol;
...
static
int url_new2(url_t* self, url_t* base_url, const char* relative_path)
{
    int i;
    int is_absolute_p = 1;
    char* base_path = NULL;
...
static
int url_equals(url_t* self, url_t* other)
{
    int ret_p = 1;
...
static
int url_is_subdir_of(url_t* self, url_t* other)
{
    int ret_p = 1;
	...
	if(self->path != other->path) {
		...
		if(dir == NULL)
			...
			while(*remaining != '\0' && (*remaining == '/' || *remaining == '.'))
			    remaining++;
