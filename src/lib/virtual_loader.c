 ...
 *
 * You should have received this file ('src/lib/virtual_loader.c') with a license
...
static inline
virtual_path_node_t* vp_instantiate()
{
    return calloc (sizeof(virtual_path_node_t), 1);
}

static inline
void vp_tear_off(virtual_path_node_t* x)
{
    if(x->type == URL) {
	if(x->content.url) {
...
static
url_t* search_path_iterator_next(local_path_iterator_t* zelf)
{
...
static
int search_path_iterator_destroy(object_t* zelf)
{
    search_path_iterator_t* self = (search_path_iterator_t*) zelf;
...
{
    search_path_iterator_t* it = search_path_iterator_instantiate_toplevel (NULL);
...
search_path_iterator_t* search_path_iterator_instantiate (search_path_iterator_t* x) {
    ...
    object_set_release_callback (search_path_iterator_to_object (it), it->super.destroy);    
    
    return it;
}

static
stream_t* indirect_open(url_t* local_url, const char* path, const char* mode)
{
    stream_t* ztrmmm;
    url_t tu;
	...
	TRACE6("url got: '%s://%s:%d%s' for path '%s'", 
	       tu.protocol, tu.server, tu.port, tu.path, path);
    else
...
static
stream_t* virtual_url_open(const char* url, const char* mode)
{
	...
	*vpath++ = '\0';
	...
	if(it) {
		...
		WARNING3("didn't find local url '%s' for host: '%s'", vpath, vhost);
	    }
	}
    }
...
static inline
virtual_path_node_t* virtual_path_get(virtual_loader_t* self, const char* vhost)
{
...
static
url_t* virtual_loader_get_local_url(virtual_loader_t* self, const char* vhost)
{
...
static
void virtual_loader_set_local_url(virtual_loader_t* self, 
...
static
local_path_iterator_t* virtual_loader_get_search_path(virtual_loader_t* self, 
						       const char* vhost)
{
    virtual_path_node_t* v = virtual_path_get(self, vhost);
...
static
void virtual_loader_add_search_path(virtual_loader_t* self, const char* vhost, url_t* url)
{
    virtual_path_node_t* v = virtual_path_get(self, vhost);
    if(url) {
	...
	/* clone url */
	TRACE5 ("vhost: %s now points to: %s %s %s", vhost, url->protocol, url->server, url->path);
