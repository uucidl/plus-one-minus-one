 ...
 *
 * You should have received this file ('src/lib/resource_loaders.c') with a
 *license
...
static unsigned int find_slot(const char **strs, const unsigned int next,
...
rloader_t *rloader_instantiate(rloader_t *x)
{
    ...
    if (l != &resource_loaders || !resource_loaders_init_p) {
