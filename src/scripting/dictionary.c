 ...
 *
 * You should have received this file ('src/scripting/dictionary.c') with a
 *license
...
static inline atom_container_t *get_atom(dictionary_t *self, const char *name,
                                         atom_container_t **pac)
{
    cstr_map_value_t aac;
    atom_t a = 0;

    aac = cstr_map_get(&self->atoms, name);
    a = (atom_t)(cstr_map_value_is_there(aac) ? cstr_map_value_obtain(aac)
                                              : NULL);
...
static atom_t dictionary_new_atom(dictionary_t *self, const char *name)
{
...
static atom_t dictionary_new_atom_from_integer(dictionary_t *self, const int i)
{
    atom_t atom = 0;
    unsigned int ii = i;
    unsigned int n = 1;
...
static atom_t dictionary_get_atom(dictionary_t *self, const char *name)
{
...
static int dictionary_release(object_t *zelf)
{
