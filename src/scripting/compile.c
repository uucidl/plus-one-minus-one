...
static
bytecode_t* parse_definition(const string_t* string, bytecode_t* code) 
{
    bytecode_t* b      = code;
    ...
    if(string->clone_to(string, &temp) && 
       temp.get_iterator(&temp, &it)) {
	...
	if(c) {
	    ...
	    TRACE3("subs: '%s' '%s'", 
		   adverb->new_c_str(adverb), verb->new_c_str(verb));
	} else {
	    verb = &temp;
	}
...
bytecode_stream_t* compile_string(const string_t* string, bytecode_stream_t* x)
{
    ...
    if(!atom_atom) atom_atom = dict->new_atom(dict, "atom");
    if(!integer_atom) integer_atom = dict->new_atom(dict, "integer");
    if(!float_atom) float_atom = dict->new_atom(dict, "float");
...
bytecode_stream_t* compile_osc_string(const string_t* osc_string, bytecode_stream_t* x)
{
    dictionary_t* dict = dictionary_get_instance();
    bytecode_stream_t* stream = NULL;
    substring_generator_t gen;
    ...
    if(!integer_atom) integer_atom = dict->new_atom(dict, "integer");
    if(!float_atom) float_atom = dict->new_atom(dict, "float");
    if(!route_atom) route_atom = dict->new_atom(dict, "route");
	...
	if((s = gen.next(&gen))) {
		...
		while((adr = route_gen.next(&route_gen))) {
		    ...
		    TRACE2("prepending atom '%s'", atom_get_cstring_value(atom));
