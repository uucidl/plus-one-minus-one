...
image_t* image_instantiate (image_t* x) {
    return image_instantiate_super (x);
}

int image_new(image_t* self, 
	      unsigned int width, 
	      unsigned int height, 
	      unsigned int pitch)
{
		       ...
		       int qwidth,  int qheight)
{
    ...
    float xa = x1; float ya = y1;
    float xb = x4; float yb = y4;
    { int j; for(j=0; j<qheight; j++) {
	/* deltas for scanline */
	...
	float x = xa;
	float y = ya;
	int i; for(i=0; i<qwidth; i++) {
#ifdef CLIP_TEST
	    if(x > 0 && x < self->width && y > 0 && y < self->height) {
#endif
	    scanline[i] = self->pixels[((int)y % self->height)*self->pitch + ((int)x % self->width)];
#ifdef CLIP_TEST
            }
#endif
	    x += dxleft_up;
